...
__kernel void flash_attn_f32(
    ...
    for (int k_start = 0; k_start < n_kv; k_start += BLOCK_N) {
        ...
        if (!has_mask && !has_softcap) {
            for (int j = 0; j < valid_even; j += 2) {
                ...
                for (int k = 0; k < DK_VEC; k++) {
                    dot_sum0 += dot(q_priv[k], CONVERT_ACC4(l_k[j][k]));
                    dot_sum1 += dot(q_priv[k], CONVERT_ACC4(l_k[j+1][k]));
                }
            ...
            for (int j = 0; j < valid_even; j += 2) {
                ...
                if (has_mask) {
                    // Pairwise mask load matches the j/j+1 processing pattern and reduces
                    // scalar half reads on the masked hot path.
                    const float2 mask_pair = vload_half2(0, mask_ptr_row + k_row0);
                    score0 += slope * (ACC_TYPE)mask_pair.s0;
                    score1 += slope * (ACC_TYPE)mask_pair.s1;
                }
                if (has_softcap) {
        ...
        if (work_cols & 1) {
            ...
            #pragma unroll
            for (int k = 0; k < DK_VEC; k++) {
                dot_sum += dot(q_priv[k], CONVERT_ACC4(l_k[j][k]));
            }
            ...
            #pragma unroll
            for (int i = 0; i < DV_VEC; ++i) {
                o_acc[i] = o_acc[i] * scale_prev + p * CONVERT_ACC4(l_v[j][i]);
            }
...
__kernel void flash_attn_f32_q1(
    ...
    if (fast_path_q1) {
        for (int k_idx = tid; k_idx < n_kv; k_idx += Q1_WG_SIZE) {
            ...
            #pragma unroll
            for (int i = 0; i < DV_VEC; i++) {
                o_acc[i] = o_acc[i] * scale_prev + p * CONVERT_ACC4(v_ptr[i]);
            }
            l_i = l_i * scale_prev + p;
            m_i = m_new;
        }
        ...
        for (int chunk = 0; chunk < DV_VEC; chunk += 2 * STAGE2_CHUNK_PAIRS) {
            ...
            if (tid == 0) {
                for (int j = 0; j < num_pairs; j++) {
                    ...
                    o_row[chunk + 2 * j]     = CONVERT_DATA4((ACC_TYPE4)(r.s0, r.s1, r.s2, r.s3) * l_inv);
                    o_row[chunk + 2 * j + 1] = CONVERT_DATA4((ACC_TYPE4)(r.s4, r.s5, r.s6, r.s7) * l_inv);
                }
            }
            barrier(CLK_LOCAL_MEM_FENCE);
        }
...
__kernel void flash_attn_f32_q1_splitk_stage1(
    const global void * q_void, ulong q_offset,
    const global void * k_void, ulong k_offset,
    const global void * v_void, ulong v_offset,
    global void * o_void, ulong o_offset,
...
__kernel void flash_attn_f32_q1_splitk_stage2(
    ...
    const ulong o_row_offset = batch_idx * o_nb3 + head_idx * o_nb1;
    global DATA_TYPE4 *o_row = (global DATA_TYPE4 *)((global char*)o_void + o_offset + o_row_offset);
    ...
    if (tid == 0) {
        ...
        if (l_sum > 0.0f) {
            for (int i = 0; i + 1 < DV_VEC; i += 2) {
                ...
                o_row[i]   = CONVERT_DATA4(s0 * l_inv);
                o_row[i + 1] = CONVERT_DATA4(s1 * l_inv);
            }
            if (DV_VEC & 1) {
                const int i = DV_VEC - 1;
                ACC_TYPE4 s0 = (ACC_TYPE4)(0.0f);
