...
function Show-Usage {
    ...
    Write-Host "Usage: $scriptName [-n | -DryRun] [-h | -Help]"
    Write-Host "  -h, -Help     show this help message"
    Write-Host "  -n, -DryRun   perform the check but dont install"
}

foreach ($arg in $args) {
    if ($arg -notin @("-DryRun", "-n", "-Help", "-h")) {
    ...
    process {
        ...
        if (Test-Path $global:tools['vswhere'] ) {
            ...
            if (!$INSTALLED_VS_VERSION) {
                if ($DryRun) {
                    Write-Host "Missing required Visual Studio Instance."
                    exit 1
                } else {
                    Install-VS-Instance
                    $INSTALLED_VS_VERSION = & $global:tools['vswhere'] -latest -property installationVersion
...
if ($global:need_installed.Count -ne 0) {
    if ($DryRun) {
        Write-Host "Missing required packages: $($global:need_installed -join ', ')"
        exit 1
    } else {
