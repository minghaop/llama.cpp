...
class _SpanFiller(ExprMutator):
    ...
    def __init__(self, span, op_type, output_names, output_qparams):
        ...
        if isinstance(span, tvm.relay.SequentialSpan):
            ...
            self._span_for_intermediate_expr = lambda distance: tvm.relay.Span(tvm.relay.SourceName(span_name), 0, 0, 0, 0, op_type, distance=distance)
        else:
    ...
    def reset_span(self):
        if self.distance == 0:
            ...
            self._span = self._span_for_intermediate_expr(self.distance)
