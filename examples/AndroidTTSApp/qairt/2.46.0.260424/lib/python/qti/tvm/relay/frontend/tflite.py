...
class OperatorConverter(object):
    ...
    ### [End QTI Change]
        ...
        self.convert_map = {
            ...
            ### [QTI Change]
            "CONV_3D_TRANSPOSE": self.convert_transpose_conv3d,
            ### [End QTI Change]
    ...
    def get_tensors(self, tensors_idx_list):
        ...
        for tensor_idx in tensors_idx_list:
            ...
            if tflite_qnn_params is not None:
                ...
                ### [QTI Change]
                tflite_quant_dim = tflite_qnn_params.QuantizedDimension()
                ### [End QTI Change]
                ...
                if is_qnn_params_valid:
                    ...
                    ### [QTI Change]
                    qnn_params["quant_dim"] = relay.const(tflite_quant_dim, "int32")
                    ### [End QTI Change]
    ...
    def get_tensor_value(self, tensor_wrapper, is_sparse=False):
        ...
        if tensor_wrapper.qnn_params is not None:
            scale = tensor_wrapper.qnn_params["scale"].data.asnumpy()
            zero_point = tensor_wrapper.qnn_params["zero_point"].data.asnumpy()
            quant_dim = tensor_wrapper.qnn_params["quant_dim"].data.asnumpy()

            # Reshape scale according to quant_dim to dequantize data
            # Ex: data = [32, 3, 3, 3], scale = [32] quant_dim = 0
            # Reshape scale into [32, 1, 1, 1] to dequantize data
            if len(scale.shape) and scale.shape[0] != 1:
                num_expand_dims = len(shape) - quant_dim - 1
                new_scale_shape = [scale.shape[0]] + [1] * num_expand_dims
                scale = np.reshape(scale, new_scale_shape)

            data = (data.astype(scale.dtype) - zero_point.astype(scale.dtype)) * scale
            dtype = scale.dtype
    ...
    def get_tensor_type_str(self, tensor_type):
        ...
        if tensor_type == TensorType.BOOL:
            return "bool"
        ### [QTI Change]
        if tensor_type == TensorType.UINT16:
            return "uint16"
        if tensor_type == TensorType.FLOAT64:
            return "float64"
        ### [End QTI Change]
        raise NotImplementedError(
    ...
    def convert_qnn_fused_activation_function(
        self, expr, fused_activation_fn, scale, zero_point, dtype, require_qdq=False
    ):
        """Convert TFLite fused activation function. The expr is an input quantized tensor with
        scale and zero point. Require_qdq is set to True when expr output value may be outside
        quantization range"""
    ### [End QTI Change]
        try:
            from tflite.ActivationFunctionType import ActivationFunctionType
            ...
            raise ImportError("The tflite package must be installed")
        ### [QTI Change]
        scale_const = relay.const(scale, "float32")
        zero_point_const = relay.const(zero_point, "int32")
        dequantized_expr = _qnn.op.dequantize(
            data=expr,
            input_scale=scale_const,
            input_zero_point=zero_point_const,
        )

        if require_qdq and fused_activation_fn == ActivationFunctionType.NONE:
            # Add clip to truncate the value outside quantization range
            activation_expr = _qnn.op.simulated_quantize(dequantized_expr, scale_const, zero_point_const, out_dtype=dtype)
            activation_expr = _qnn.op.simulated_dequantize(activation_expr, scale_const, zero_point_const, in_dtype=dtype)
        elif fused_activation_fn == ActivationFunctionType.RELU:
            # Keep relu node with quantization range and let QNN/Backend handle the relu optimization
            activation_expr = _qnn.op.simulated_quantize(dequantized_expr, scale_const, zero_point_const, out_dtype=dtype)
            activation_expr = _qnn.op.simulated_dequantize(activation_expr, scale_const, zero_point_const, in_dtype=dtype)
            activation_expr = _op.nn.relu(activation_expr)
        else:
            activation_expr =  self.convert_fused_activation_function(dequantized_expr, fused_activation_fn)

        out_expr = _qnn.op.quantize(
            data=activation_expr,
            output_scale=scale_const,
            output_zero_point=zero_point_const,
            out_dtype=dtype,
        )
        return out_expr
        ### [End QTI Change]
    ...
    def convert_relu_n1_to_1(self, op):
        ...
        if input_tensor.qnn_params:
            ...
            out = self.convert_qnn_fused_activation_function(
                expr=in_expr,
                fused_activation_fn=ActivationFunctionType.RELU_N1_TO_1,
                scale=scale_val,
                zero_point=zero_point_val,
                dtype=input_tensor_type_str,
            )
        ### [End QTI Change]
        else:
    ...
    def convert_gather_nd(self, op):
        ...
        ### [QTI Change]
        assert indices_type in (TensorType.INT32, TensorType.INT64, TensorType.INT16)
        ### [End QTI Change]
    ...
    ### [QTI Change]
    def convert_atan2(self, op):
        """Convert TFLite ATAN2"""
        input_tensors = self.get_input_tensors(op)
        assert len(input_tensors) == 2, "input tensors length should be == 2"

        out = _op.atan(self._convert_elemwise(_op.divide, op, self.is_quantized(op)))
        return out
    ### [End QTI Change]
    ...
    ### [QTI Change]
    def convert_conv3d(self, op, conv_type="conv3d"):
        """convolution implementation."""
        try:
            from tflite.BuiltinOptions import BuiltinOptions
        ...
        if conv_type == "conv3d":
            ...
            op_options = op.BuiltinOptions()
            conv_options = Conv3DOptions()
            conv_options.Init(op_options.Bytes, op_options.Pos)
        elif conv_type == "depthwiseconv3d":
            is_depthwise_conv = True
            assert op.BuiltinOptionsType() == BuiltinOptions.DepthwiseConv3DOptions
            op_options = op.BuiltinOptions()
            conv_options = DepthwiseConv3DOptions()
            conv_options.Init(op_options.Bytes, op_options.Pos)
            depth_multiplier = conv_options.DepthMultiplier()
        else:
            raise tvm.error.OpNotImplemented(
        ...
        stride_h = conv_options.StrideH()
        stride_w = conv_options.StrideW()
        stride_d = conv_options.StrideD()
        dilation_h = conv_options.DilationHFactor()
        dilation_w = conv_options.DilationWFactor()
        dilation_d = conv_options.DilationDFactor()
        padding = conv_options.Padding()
        fused_activation_fn = conv_options.FusedActivationFunction()
        ...
        if is_depthwise_conv:
            # TFLite depthwise convolution kernel layout is:
            # KD KH KW C(input_c * depth_multiplier)
            _, kernel_d, kernel_h, kernel_w, in_channels = to_int_list(self.get_tensor_shape(weight_tensor))
            assert in_channels == input_c * depth_multiplier
        else:
            output_channels, kernel_d, kernel_h, kernel_w, in_channels = to_int_list(
                self.get_tensor_shape(weight_tensor)
            )
        ...
        if is_depthwise_conv:
            ...
            # If number of input channels is 1, treat as normal
            # convolution.
            params["kernel_layout"] = "DHWIO" if input_c == 1 else "DHWOI"
        else:
            params["channels"] = int(output_channels)
        ...
        if self.has_expr(weight_tensor.tensor_idx):
            ...
            if is_depthwise_conv:
                weight_expr = _op.reshape(
                    weight_expr, (kernel_d, kernel_h, kernel_w, input_c, depth_multiplier)
                )
            else:
                weight_expr = _op.transpose(weight_expr, axes=(1, 2, 3, 0))
        else:
            ...
            # KH KW IC M (depth_multiplier) (HWOI)
            if is_depthwise_conv:
                weight_value = weight_value.reshape(kernel_d, kernel_h, kernel_w, input_c, depth_multiplier)
            else:
                weight_value = weight_value.transpose((1, 2, 3, 4, 0))
        ...
        if padding == Padding.VALID:
            ...
            pad_left, pad_right = get_pad_value(input_w, dilated_kernel_w, stride_w)
            pad_d1, pad_d2 = get_pad_value(input_d, dilated_kernel_d, stride_d)
            do_pad = not (pad_top == 0 and pad_bottom == 0 and pad_left == 0 and pad_right == 0)
            if do_pad:
                params["padding"] = [pad_d1, pad_d2, pad_top, pad_left, pad_bottom, pad_right]
        ...
        if input_tensor.qnn_params:
            qnn_conv3d_params = dict(params)
            qnn_conv3d_params["input_zero_point"] = input_tensor.qnn_params["zero_point"]
            qnn_conv3d_params["kernel_zero_point"] = weight_tensor.qnn_params["zero_point"]
            qnn_conv3d_params["out_dtype"] = (
                "int64" if output_tensor_type_str == "int16" else "int32"
            )
            qnn_conv3d_params["input_scale"] = input_tensor.qnn_params["scale"]
            qnn_conv3d_params["kernel_scale"] = weight_tensor.qnn_params["scale"]
            out = _qnn.op.conv3d(in_expr, weight_expr, **qnn_conv3d_params)
        else:
            out = _op.nn.conv3d(in_expr, weight_expr, **params)
    ...
    def convert_cumsum(self, op):
        """Convert TFLite CUMSUM Op"""
        input_tensors = self.get_input_tensors(op)
        assert len(input_tensors) == 2, "input tensors length should be == 2"
        input_tensor = input_tensors[0]
        in_expr = self.get_expr(input_tensor.tensor_idx)

        in_axis = self.get_tensor_value(input_tensors[1]).item()

        out = _op.cumsum(in_expr, in_axis)
    ...
    ### [QTI Change]
    def convert_sign(self, op):
        """Convert TFLite SIGN"""
        return self._convert_unary_elemwise(_op.sign, op)
    ### [End QTI Change]
    ...
    def convert_pad(self, op):
        ...
        if input_tensor.qnn_params:
            ...
            ### [QTI Change]
            # Remove to get dequantized pad_value
            ### [End QTI Change]
    ...
    def convert_transpose_conv(self, op):
        ...
        ### [QTI Change]
        # Get fused activation function from options
        fused_activation_fn = deconv_options.FusedActivationFunction()
        ### [End QTI Change]
        ...
        if output_tensor.qnn_params:
            ...
            ### [QTI Change]
            # Call activation function for quantized case
            output_scale_val = get_scalar_from_constant(output_tensor.qnn_params["scale"])
            output_zero_point_val = get_scalar_from_constant(output_tensor.qnn_params["zero_point"])
            out = self.convert_qnn_fused_activation_function(
                expr=out,
            ...
            # Call activation function for non-quantized case
            out = self.convert_fused_activation_function(out, fused_activation_fn)
            ### [End QTI Change]
    ...
    ### [QTI Change]
    def convert_transpose_conv3d(self, op):
        """Convert TFLite TRANSPOSE_CONV_3D"""
        try:
            from tflite.BuiltinOptions import BuiltinOptions
            from tflite.Padding import Padding
            from tflite.TensorType import TensorType
        ...
        # Input (data) Tensor. NDHWC layout
        input_tensor = input_tensors[2]
        _, depth, height, width, input_c = to_int_list(self.get_tensor_shape(input_tensor))

        # Weights tensor. ODHWI layout in TFLite
        weights_tensor = input_tensors[1]
        out_channels, kernel_d, kernel_h, kernel_w, in_channels = to_int_list(
            self.get_tensor_shape(weights_tensor)
        )

        assert (
                input_c == in_channels
        ), "Input channel in the filter should match the channel in the input"
        ...
        output_tensor_type = output_tensor.tensor.Type()
        output_tensor_type_str = self.get_tensor_type_str(output_tensor_type)
        output_shape_value = list(output_tensor.tensor.ShapeAsNumpy())
        ...
        stride_h = deconv_options.StrideH()
        stride_w = deconv_options.StrideW()
        fused_activation_fn = deconv_options.FusedActivationFunction()  # Extract fused activation

        # TFLite TransposeConvOptions lacks stride for the depth (D) dimension.
        # Assuming stride_d = stride_h for 3D transpose conv.
        # Consider verifying the original model for Non-uniform strides (e.g., [1, 2, 2]).

        # Assumed due to lack of explicit stride_d in TFLite schema
        stride_d = stride_h

        logger.warning("Assuming stride_d = stride_h (%d) for TRANSPOSE_CONV_3D. "
               "TFLite schema does not provide stride_d explicitly."
               "Verify that the original model used uniform strides to avoid shape "
               "mismatches.",stride_h)

        assert padding in (
            Padding.VALID,
            Padding.SAME,
        ), f"Padding format {padding} is not supported for operator TRANSPOSE_CONV_3D"

        # Data layout adjustments for NDHWC
        in_expr = self.get_expr(input_tensor.tensor_idx)

        # Weights layout adjustment from ODHWI to IODHW
        weights_tensor_type = weights_tensor.tensor.Type()
        # weights tensor type should be UINT8 (quantization) or FLOAT32
        assert weights_tensor_type in (TensorType.INT8, TensorType.UINT8, TensorType.FLOAT32)
        weight_tensor_type_str = self.get_tensor_type_str(weights_tensor_type)
        ...
        if self.has_expr(weights_tensor.tensor_idx):
            weight_expr_iodhw = self.get_expr(weights_tensor.tensor_idx)
            weight_expr_iodhw = _op.transpose(weight_expr_iodhw, axes=(4, 0, 1, 2, 3))
        else:
            weight_value_odhwi = self.get_tensor_value(weights_tensor)
            weight_value_iodhw = np.transpose(weight_value_odhwi, (4, 0, 1, 2, 3))
            weight_expr_iodhw = self.exp_tab.new_const(
                weight_value_iodhw,
                dtype=weight_tensor_type_str,
                source_name=weights_tensor.tensor.Name(),
            )

        # Handle padding for depth, height, and width
        if padding == Padding.SAME:
            output_d, output_h, output_w = output_shape_value[1], output_shape_value[2], output_shape_value[3]
            pad_front, pad_back = get_pad_value(output_d, kernel_d, stride_d)
            pad_top, pad_bottom = get_pad_value(output_h, kernel_h, stride_h)
            pad_left, pad_right = get_pad_value(output_w, kernel_w, stride_w)
            padding = (pad_front, pad_top, pad_left, pad_back, pad_bottom, pad_right)
        else:
            padding = (0, 0, 0, 0, 0, 0)
    ...
    def convert_custom(self, op, node_name, custom_ops_dict):
        from qti.aisw.converters.qnn_backend.custom_ops.core import qnn_to_dtype, convert_to_backend_type_from_numpy
        ...
        def custom_op_rel(arg_types, attrs):
            ...
            # After dequantization, the arg_types may change.
            # Update input/output data types in custom_tflite_op accordingly.

            for arg, input_tensor in zip(arg_types, custom_tflite_op.inputs):
                arg_dtype = convert_to_backend_type_from_numpy(np.dtype(arg.dtype))
                if arg_dtype != input_tensor.data_type and arg_dtype in input_tensor.allowed_data_types:
                    input_tensor.data_type = arg_dtype

            # Set output dtype based on the first input's dtype from arg_types
            output_dtype = convert_to_backend_type_from_numpy(np.dtype(arg_types[0].dtype))
            for output_tensor in custom_tflite_op.outputs:
                if output_dtype != output_tensor.data_type and output_dtype in output_tensor.allowed_data_types:
                    output_tensor.data_type = output_dtype
...
### [End QTI Change]
    ...
    for model_input_name in model_inputs:
        ...
        # Validate if the input name exists in the model
        if tensor_idx is None:
            raise KeyError(f"Invalid input name '{model_input_name}' provided. "
                           f"Please check the model's input names.")
        tensor = subgraph.Tensors(tensor_idx)
