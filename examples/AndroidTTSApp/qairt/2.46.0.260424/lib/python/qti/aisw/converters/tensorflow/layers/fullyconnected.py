...
class FullyConnectedLayerResolver(LayerResolver, object):
    ...
    def __init__(self):
        ...
        sequence = GraphSequence([
            ...
            ConverterSequenceNode('bias_op', ['BiasAdd', 'Add', 'AddV2']),  # output
        ...
        for sequence in self.sequences:
            ...
            for match in matches:
                ...
                if len(weights_shape) != 2 or (
                        weights_op.type not in ['Identity', 'Const', 'Split', 'FakeQuantWithMinMaxVars'] and \
                        not graph_helper.check_op_const_origin(weights_op)[0]):
                    # If weights is not Constant or 2D tensor, we cannot convert Matmul to FullyConnected.
                ...
                if biases_op is not None and bias_add_op is not None:
                    if biases_op.type not in ['Identity', 'Const']:
                        ...
                        raise ConverterError(
                            code_to_message.get_error_message('ERROR_TF_MATMUL_RESOLVE_BIAS')
                            (str(matmul_op.name), bias_add_op.name))
