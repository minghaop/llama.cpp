...
class L1ErrorVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            ...
            return VerifyResult(layer_type, len(inference_raw), tensor_dimensions, L1ErrorVerifier.L1_NAME, msg, "")
        ...
        # Handle nan value
        l1_error = 'NaN' if np.isnan(l1_error) else l1_error
        ...
        if self.mean:
            return VerifyResult(layer_type, len(inference_raw), tensor_dimensions,
