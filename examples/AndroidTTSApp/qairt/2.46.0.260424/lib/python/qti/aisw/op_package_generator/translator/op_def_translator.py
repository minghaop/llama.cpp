...
class XmlConstants:
    ...
    QUANT_FLOAT_ENCODING = "FloatEncoding"
    QUANT_SCALE_DATATYPE = "ScaleDatatype"
...
class OpDefTranslator:
    ...
    def translate_ops(self, op_def_collection: OpDefCollection=None) -> OpDefCollection:
        ...
        if root.tag == XmlConstants.OP_DEF_COLLECTION:
            ...
            # Parse UseQHPI at the OpDefCollection level (it's an attribute, not an element)
            use_qhpi_attr = root.get(XmlConstants.USE_QHPI)
            if use_qhpi_attr is not None:
                op_def_collection.use_qhpi = self.__text_to_bool(use_qhpi_attr)
    ...
    def get_op_def_xml(self, op_def_collection: OpDefCollection) -> ET.ElementTree:
        ...
        # Add UseQHPI at the OpDefCollection level if set (as an attribute)
        if hasattr(op_def_collection, 'use_qhpi') and op_def_collection.use_qhpi:
            root_elem.set(XmlConstants.USE_QHPI, "true")
    ...
    def __create_input(self, input_elem: ET.Element) -> InputElement:
        ...
        # Add parsing for IsQuantizable
        if input_elem.find(XmlConstants.IS_QUANTIZABLE) is not None:
            is_quantizable = self.__text_to_bool(input_elem.find(XmlConstants.IS_QUANTIZABLE).text)
    ...
    def __translate_xml_quant_params(self, quant_param_elem: List[ET.Element], quant_params: list):
        ...
        if quant_param_elem is not None:
            for param in quant_param_elem:
                ...
                #Bitwidth
                bw = None
                bw_elem = param.find(XmlConstants.QUANT_BITWIDTH)
                ...
                if scale_elem is not None:
                    ...
                    # Changing xml fraction string to float value
                    if scale_elem.text.find('/') != -1:
                        divider = scale_elem.text.find('/')
                        scale = float(scale_elem.text[:divider]) / float(scale_elem.text[divider+1:])
                    else:
                ...
                if math_invariant_elem is not None:
                    math_invariant = self.__text_to_bool(math_invariant_elem.text)

                # Mapping
                mapping = None
                mapping_elem = param.find(XmlConstants.QUANT_MAPPING)
                if mapping_elem is not None:
                    mapping = EncodingMappingType[mapping_elem.text]

                # FloatEncoding
                float_encoding = None
                float_encoding_elem = param.find(XmlConstants.QUANT_FLOAT_ENCODING)
                if float_encoding_elem is not None:
                    float_encoding = FloatEncodingType[float_encoding_elem.text]

                # ScaleDatatype
                scale_datatype = None
                scale_datatype_elem = param.find(XmlConstants.QUANT_SCALE_DATATYPE)
                if scale_datatype_elem is not None:
                    scale_datatype = QnnDatatype[scale_datatype_elem.text]

                quant_params.append(QuantParam(encodings, bw, axis, symmetric, scale, offset, min, max, \
                                               math_invariant, mapping, float_encoding, scale_datatype))
    ...
    def __translate_quant_params_to_xml(self, xml_elem: ET.Element, supp_op_def_attr: OpDefElement):
        ...
        for param in supp_op_def_attr.quant_params:
            ...
            #Mapping
            if param.mapping != "":
                quant_param.set(XmlConstants.QUANT_MAPPING, param.mapping.name)

            #FloatEncoding
            if param.float_encoding != "":
                quant_param.set(XmlConstants.QUANT_FLOAT_ENCODING, param.float_encoding.name)

            #ScaleDatatype
            if param.scale_datatype != "":
                quant_param.set(XmlConstants.QUANT_SCALE_DATATYPE, param.scale_datatype.name)
