...
from qti.aisw.accuracy_debugger.lib.utils.nd_framework_utility import dequantize_tflite_tensor
...
class TFLiteFramework_2_2_0(BaseFramework):
    ...
    def run_inference(self, input_data: list, input_tensor_names: list, output_tensor_names: list,
        ...
        for i, output_index in enumerate(output_tensor_indices):
            ...
            raw_output = self._interpreter.get_tensor(output_index)

            # Dequantize if needed (unless use_native_output_files is True)
            if not use_native_output_files:
                # Get tensor details for dequantization
                tensor_details = self._interpreter.get_tensor_details()
                tensor_detail = next((t for t in tensor_details if t['index'] == output_index), None)
                raw_output = dequantize_tflite_tensor(raw_output, tensor_detail)

            results[output_tensor_names[i]] = raw_output
        ...
        while (len(stack) != 0):
            ...
            if (curr_idx not in op_list):
                ...
                for i in range(operator.InputsLength()):
                    ...
                    # as an output:
                    input_idx = operator.Inputs(i)
                    adj_op = -1 if input_idx == -1 else self._tensor_to_op_map[input_idx]
                    if (adj_op != -1):
                        stack.append(adj_op)
