...
from qti.aisw.emitter.utils import torch_utils
try:
    import aimet_torch.nn.modules.custom as aimet_modules
except ImportError:
    from qti.aisw.emitter import elementwise_ops as aimet_modules

_logger = logging.getLogger('OnnxSaver')
...
class OnnxSaver:
    ...
    def _add_markers(
        ...
        for local_module_name, module_ref in starting_module.named_children():
            if torch_utils.is_leaf_module(module_ref):
    ...
    def _create_onnx_model_with_markers(
        ...
        for module_name, module_ref in model.named_modules():
            if add_all_markers or torch_utils.is_leaf_module(module_ref):
    ...
    def _export_model_to_onnx(
        ...
        if is_conditional:
            with torch_utils.in_eval_mode(model), torch.no_grad():
