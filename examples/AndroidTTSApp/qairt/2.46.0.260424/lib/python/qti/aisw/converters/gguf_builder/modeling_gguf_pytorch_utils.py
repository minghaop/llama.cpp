...
# Import from local ggml.py file
...
GGUF_TO_TRANSFORMERS_MAPPING = {
    ...
    "ggml_types": {},
    "config_rope": {
        "long_factor": "rope_factors_long.weight",
        "short_factor": "rope_factors_short.weight",
    }
}
...
def load_gguf_checkpoint(gguf_checkpoint_path: object, return_tensors: object = False,
    ...
    for k, v in GGUF_TO_TRANSFORMERS_MAPPING["config_rope"].items():
        tensor_info = [t for t in reader.tensors if t.name == v]
        if tensor_info:
            parsed_parameters["config_rope"][k] = np.asarray(tensor_info[0].data).tolist()

    architecture = read_field(reader, "general.architecture")[0]
    model_name = []
    for key in ("general.name", "general.basename"):
        if key in reader.fields.keys():
            model_name.append(read_field(reader, key)[0])

    # in llama.cpp mistral models use the same architecture as llama. We need
    # to add this patch to ensure things work correctly on our side.
    if "llama" in architecture and any(v in model_name[0].lower() for v in MISTRAL_VARIANTS):
    ...
    if return_tensors:
        ...
        for tensor in tqdm(reader.tensors, desc="Converting and de-quantizing GGUF tensors..."):
            ...
            if architecture in ["llama", "mistral"] and (".attn_k." in name or ".attn_q." in name):
                ...
                if ".attn_q." in name:
                    ...
                    if encodings and encodings.get("scale", None):
            ...
            if extract_encodings:
                if encodings:
                    ...
                    parsed_parameters["ggml_types"][name] = tensor.tensor_type.value
...
def reverse_permute_helper(data: np.ndarray, in_shape: np.ndarray, n_head: int) -> list:
    data_shape = data.shape[0]
    data = data.reshape(in_shape[0], data_shape // in_shape[0])
    dim = data.shape[0] // n_head // 2
    w = data.reshape(n_head, dim, 2, *data.shape[1:])
    return w.swapaxes(2, 1).reshape(data.shape).flatten().tolist()
...
def reverse_permute_encodings(encodings: dict, shape: np.ndarray, n_head: int, num_kv_heads: Optional[int] = None) -> dict:
    # Reverse permutation implementation for q/k encodings for llama models 
    if num_kv_heads is not None and n_head != num_kv_heads:
        n_head = num_kv_heads
    encodings['scale']  = reverse_permute_helper(np.array(encodings['scale']), shape, n_head)
    encodings['offset'] = reverse_permute_helper(np.array(encodings['offset']), shape, n_head)
