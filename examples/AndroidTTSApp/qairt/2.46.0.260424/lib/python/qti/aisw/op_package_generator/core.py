...
class QnnTemplateFileReader:
    ...
    # QHPI template files for HTP backend when UseQHPI is true
    # Note: HTPMCP is excluded from plugin templates as it doesn't support QHPI
    PLUGIN_TEMPLATE_FILES = {
        TemplateFileTypes.HTP: ['htp_plugin_source.mako', "htp_plugin_interface.mako"],
        TemplateFileTypes.HTP_FP16: ['htp_plugin_source.mako', "htp_plugin_interface.mako"],
    }

    @classmethod
    def get_template_type_by_backend(cls, backend, use_plugin=False):
        """
        Get template files for a specific backend.

        Args:
            backend (str): The backend name (e.g., 'HTP', 'CPU', etc.)
            use_plugin (bool): Whether to use QHPI templates instead of DEF_* templates

        Returns:
            List[str]: List of template file names for the backend
        """
        if use_plugin and backend in ['HTP', 'HTP_FP16']:
            backend_enum = getattr(cls.TemplateFileTypes, backend)
            if backend_enum in cls.PLUGIN_TEMPLATE_FILES:
                return cls.PLUGIN_TEMPLATE_FILES[backend_enum]
...
class QnnPackageInfo:
    ...
    def __init__(self, package_name, package_root, backend=None, operators=None, version='1.0.0', domain='aisw', use_qhpi=False):
