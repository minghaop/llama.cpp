...
#   Conv3D_Transpose
# ------------------------------------------------------------------------------
class RelayConv3DTransposeTranslation(RelayTranslationBase):
    def __init__(self):
        super(RelayConv3DTransposeTranslation, self).__init__()
    ...
    def extract_attributes(self,
        ...
        log_debug3("\tdata layout {}", conv_attrs.data_layout)
        if conv_attrs.data_layout != "NDHWC":
            # QUIR expects data to be "NDHWC"
            raise ValueError("Unsupported data layout {}".format(conv_attrs.data_layout))

        log_debug3("\tkernel layout {}", conv_attrs.kernel_layout)
        if conv_attrs.kernel_layout not in ["IODHW", "DHWIO"]:
            raise ValueError("Unsupported kernel layout {}".format(conv_attrs.kernel_layout))
        attr_dict["kernel_layout"] = conv_attrs.kernel_layout
        ...
        log_debug3("\tpadding {}", padding)
        if len(padding) == 1:
            pad_left = pad_right = pad_top = pad_bottom = pad_front = pad_back = padding[0]
        elif len(padding) == 2:
            pad_top = pad_bottom = pad_front = pad_back = padding[0]
            pad_left = pad_right = padding[1]
        elif len(padding) == 4 or len(padding) == 6:
            pad_top = padding[0]
            pad_left = padding[1]
            pad_bottom = padding[2]
            pad_right = padding[3]
        ...
        attr_dict["pad_left"] = pad_left
        attr_dict["pad_right"] = pad_right
        attr_dict["pad_front"] = pad_front
        attr_dict["pad_back"] = pad_back
        ...
        dilation_y = dilation[0]
        dilation_x = dilation[1]
        dilation_z = dilation[2]
        attr_dict["dilation_x"] = dilation_x
        attr_dict["dilation_y"] = dilation_y
        attr_dict["dilation_z"] = dilation_z
        ...
        output_padding = conv_attrs.output_padding
        log_debug3("\toutput padding {}", conv_attrs.output_padding)
        # FIXME: This attribute can have 1, 2, 4 or 6 numbers.
        # refer to tvm_src/include/tvm/relay/attrs/nn.h, Conv3DTransposeAttrs.
        attr_dict["output_padding_y"] = output_padding[0]
        attr_dict["output_padding_x"] = output_padding[1]
        attr_dict["output_padding_z"] = output_padding[2]
    ...
    def translate_op(self,
                     ...
                     attr_dict: dict,
                     input_names: list):
        op_name = converter_context.get_op_name(relay_expr, TransposeConv3dOp.TRANSLATION_KEY,
                                                TransposeConv3dOp.LEGACY_TRANSLATION_KEY)

        kernel_layout = attr_dict["kernel_layout"]
        if quir_graph.has_buffer(input_names[1]):
            if kernel_layout == 'IODHW':
                transpose_op_name = input_names[1] + '.dhwio'
                if not quir_graph.has_buffer(transpose_op_name):
                    transpose_op = TransposeOp(transpose_op_name, AxisTracker.AxisFormat.IODHW_TO_DHWIO)
                    quir_graph.add(transpose_op, [input_names[1]], [transpose_op_name], axis_formats=[AxisTracker.AxisFormat.DHWIO])
            ...
            if kernel_layout == 'IODHW':
                weights = np.transpose(weights, AxisTracker.AxisFormat.IODHW_TO_DHWIO)

            weights = np.ascontiguousarray(weights)
            log_debug3("\ttransposed deconv weights to {}", weights.shape)

            weights_op = ConstantOp(input_names[1], tensor=weights)
            quir_graph.add(weights_op, [], [input_names[1]], axis_formats=[AxisTracker.AxisFormat.DHWIO])
            self.populate_quantization_params(relay_expr.args[1], converter_context, quir_graph, [weights_op.name], is_param=True)
        else:
            raise ValueError("weights not found for conv3d_transpose op")
        ...
        ir_op = TransposeConv3dOp(op_name,
                                  padx_before=attr_dict["pad_left"],
                                  padx_after=attr_dict["pad_right"],
                                  pady_before=attr_dict["pad_top"],
                                  pady_after=attr_dict["pad_bottom"],
                                  padz_before=attr_dict["pad_front"],
                                  padz_after=attr_dict["pad_back"],
                                  stridex=attr_dict["stride_x"],
                                  stridey=attr_dict["stride_y"],
                                  stridez=attr_dict["stride_z"],
                                  dilationx=attr_dict["dilation_x"],
                                  dilationy=attr_dict["dilation_y"],
                                  dilationz=attr_dict["dilation_z"],
                                  output_paddingx=attr_dict["output_padding_x"],
                                  output_paddingy=attr_dict["output_padding_y"],
                                  output_paddingz=attr_dict["output_padding_z"],
                                  groups=attr_dict["groups"],
                                  padding_size_strategy=attr_dict["padding_size_strategy"])

        return ir_op

RelayTranslations.register_translation(RelayConv3DTransposeTranslation(),
                                       converter_type('conv3d_transpose', 'relay'))
    ...
    def translate_op(self,
        ...
        if input_names[1] in relay_params:
            ...
            if not quir_graph.has_buffer(input_names[1]):
                ...
                self.populate_quantization_params(relay_expr.args[1], converter_context, quir_graph, [input_names[1]], is_param=True)
                weights_node = quir_graph.add(weights_constant_op, [], [input_names[1]], axis_formats=[AxisTracker.AxisFormat.ANY])
