...
from qti.aisw.converters.relay.utils import get_prim_type, get_key_from_expr, get_dtype_from_code
...
class RelayCastTranslation(RelayTranslationBase):
    ...
    def extract_attributes(self,
        ...
        attr_dict["from_dtype"] = run_infer_type(relay_expr.args[0]).checked_type.dtype
...
class RelayDequantizeTranslation(RelayTranslationBase):
    ...
    def extract_attributes(self,
        ...
        # In qnn.dequantize, inputs[0] is the quantized input data, so we can get dtype from args[0].
        # In qnn.simulated_dequantize, inputs[0] is the quantized input data in floating-point dtype,
        # and inputs[1] stores the input dtype as code, so we can get input dtype from args[1].
        if relay_expr.op.name == 'qnn.dequantize':
            attr_dict['dtype'] = run_infer_type(relay_expr.args[0]).checked_type.dtype
        else:
            attr_dict['dtype'] = get_dtype_from_code(relay_expr.args[1])
        m = re.search(r'\d+$', attr_dict['dtype'])
    ...
    def translate_op(self,
        ...
        # Since qnn.simulated_dequantize has additional inputs[1] for input dtype,
        # remove it from the input_names for get_quantization_params function.
        if relay_expr.op.name == "qnn.simulated_dequantize":
            input_names.remove(input_names[1])
        q_params = RelayQuantization.get_quantization_params(
            op_name=op_name,
            input_names=input_names,
            relay_params=relay_params,
            attrs=attr_dict,
            use_quantize_v2=quir_graph.use_quantize_v2
        )
        ...
        converter_context.expr_to_source_info_dict[key].set_encodings([q_params])

        return DequantizeOp(op_name,
                            bw=q_params['bitwidth'],
                            min=q_params['min'],
                            max=q_params['max'],
                            scale=q_params['scale'],
                            offset=q_params['offset'])


RelayTranslations.register_translation(RelayDequantizeTranslation(),
                                       converter_type('qnn.dequantize', 'relay'),
                                       converter_type('qnn.simulated_dequantize', 'relay'))
...
class RelayQuantizeTranslation(RelayTranslationBase):
    ...
    def extract_attributes(self,
        ...
        attr_dict = {}
        # In qnn.quantize, output dtype is stored in quant_attrs.out_dtype,
        # In qnn.simulated_quantize, output dtype is stored in inputs[1] as code.
        if relay_expr.op.name == "qnn.quantize":
    ...
    def translate_op(self,
        ...
        # Since qnn.simuulated_quantize has additional inputs[1] for output dtype,
        # remove it from the input_names for get_quantization_params function.
        if relay_expr.op.name == "qnn.simulated_quantize":
            input_names.remove(input_names[1])
        q_params = RelayQuantization.get_quantization_params(
            op_name=op_name,
            input_names=input_names,
...
RelayTranslations.register_translation(RelayQuantizeTranslation(),
                                       ...
                                       converter_type('qnn.simulated_quantize', 'relay'))
