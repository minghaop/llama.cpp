...
class OnnxSparseConvTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        params = extract_attributes(src_op, attr_infos=
                                    [('stride', 'li', default_strides),
                                     ('padding', 'li', default_pads),
                                     ('dilation', 'li', default_dilations),
                                     ('kernel_size', 'li'),
                                     ('subm', 'i', 1)])
        ...
        if len(weights_constant_op.tensor.shape) == 4:
            weights_constant_op.tensor = np.transpose(weights_constant_op.tensor, (0, 3, 1, 2))
        if len(weights_constant_op.tensor.shape) == 5:
        ...
        if weights_constant_op and not graph.has_buffer(input_names[1]):
            if params.kernel_size:
                log_assert(tuple(params.kernel_size) == weights_constant_op.tensor.shape[2:],
                           "kernel_shape parameter differs from weights shape for SparseConv op {}",
                            src_op.name)
            ...
            if len(weights_constant_op.tensor.shape) == 4:
                ...
                raise NotImplementedError(
                    "Sparse Convolutional Tensor doesn't support {}-D translations.".format(len(weights_constant_op.tensor.shape)))

            self.insert_constant_trace_info(input_names[1], weights_node, converter_context)

        elif graph.has_buffer(input_names[1]):
            raise ValueError("Unsupported data format for weight tensors of {} op {}".format(src_op.op_type, src_op.name))
        ...
        if input_buffer.rank() == 5:
            ...
            return op_adapter.Conv3dOp(src_op.name,
            ...
            return op_adapter.Conv2dOp(src_op.name,
                                       bias_op_name=bias_op_name,
                                       pady_before=pad_y[0],
                                       pady_after=pad_y[1],
                                       padx_before=pad_x[0],
                                       padx_after=pad_x[1],
                                       padding_size_strategy=padding_size_strategy,
                                       stridex=params.stride[1],
                                       stridey=params.stride[0],
                                       dilationx=params.dilation[1],
                                       dilationy=params.dilation[0],
                                       data_layout=AxisTracker.AxisFormat.NCS,
                                       reuse_sparse_indices=self.reuse_sparse_indices)
    ...
    def add_op(self, src_op, context, **kwargs):
        ...
        if input_buffer.rank() == 5:
            op_axis_format = AxisTracker.AxisFormat.NCDHW
        elif input_buffer.rank() == 4:
            op_axis_format = AxisTracker.AxisFormat.NCS
        ...
        if params.activation != "None":
            ...
            node = graph.add(op, input_names, [conv_op_output], axis_formats=[op_axis_format], sparse_params=sparse_params)
            neuron_node = graph.add(neuron_op, [conv_op_output], output_names,
                                        axis_formats=[op_axis_format], sparse_params=sparse_params)
            ...
            node = graph.add(op, input_names, output_names, axis_formats=[op_axis_format], sparse_params=sparse_params)
