...
class ScaledDiffVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            msg = get_message("ERROR_VERIFIER_SIZE_MISMATCH")(str(len(golden_raw)),
                                                                 (str(len(inference_raw))))
            return VerifyResult(layer_type, len(inference_raw), tensor_dimensions, ScaledDiffVerifier.NAME, msg, "")
        ...
        # Handle nan value
        scaled_diff = 'NaN' if np.isnan(scaled_diff) else scaled_diff
        ...
        return VerifyResult(layer_type, len(inference_raw), tensor_dimensions, ScaledDiffVerifier.NAME,
