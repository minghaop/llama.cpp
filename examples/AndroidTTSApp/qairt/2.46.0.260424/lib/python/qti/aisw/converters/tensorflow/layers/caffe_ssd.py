...
class CaffeSsdLayerResolver(LayerResolver, object):
    ...
    def convert_centroids_to_corner(tensor):
        ...
        # code used as-is from implementationq
        ind = 0
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            ...
            keep_top_k = self._resolve_keep_top_k(graph_matcher, graph_helper, ssd_op.name)
            nms_top_k = self._resolve_nms_top_k(graph_matcher, graph_helper, ssd_op.name)
            score_threshold = self._resolve_score_threshold(graph_matcher, graph_helper, ssd_op.name)
            iou_threshold = self._resolve_iou_threshold(detection_out_ops_map, ssd_scope, ssd_op.name)
            ...
            if output_dims[0][-1] == 6:
                ...
                raise ConverterError(get_error_message('ERROR_TF_SSD_CAFFE_CAN_NOT_RESOLVE_OUTPUT_DIM')(ssd_op.name))
    ...
    @staticmethod
    def _resolve_keep_top_k(graph_matcher, graph_helper, ssd_op_name):
        keep_top_k = [n.original_node for n in graph_matcher.graph if n.identifier.startswith("top_k")]
        log_assert(len(keep_top_k) == 1, get_error_message('ERROR_TF_SSD_CAFFE_CAN_NOT_RESOLVE_KEEP_TOP_K')(ssd_op_name))
        return graph_helper.evaluate_tensor_output(keep_top_k[0].outputs[0]).item()

    @staticmethod
    def _resolve_nms_top_k(graph_matcher, graph_helper, ssd_op_name):
        keep_top_k = [n.original_node for n in graph_matcher.graph if n.identifier.startswith("nms_max_output_size")]
        log_assert(len(keep_top_k) == 1, get_error_message('ERROR_TF_SSD_CAFFE_CAN_NOT_RESOLVE_NMS_TOP_K')(ssd_op_name))
        return graph_helper.evaluate_tensor_output(keep_top_k[0].outputs[0]).item()

    @staticmethod
    def _resolve_score_threshold(graph_matcher, graph_helper, ssd_op_name):
        ...
        log_assert(len(score_threshold) == 1, get_error_message('ERROR_TF_SSD_CAFFE_CAN_NOT_RESOLVE_SCORE_THRESHOLD')(ssd_op_name))
        return graph_helper.evaluate_tensor_output(score_threshold[0].outputs[0]).item()

    @staticmethod
    def _resolve_iou_threshold(detection_out_ops_map, ssd_scope, ssd_op_name):
        ...
        raise ConverterError(get_error_message('ERROR_TF_SSD_CAFFE_CAN_NOT_RESOLVE_IOU_THRESHOLD')(ssd_op_name))
    ...
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        for i, name in enumerate(self.get_input_names(converter_context, descriptor, input_descriptors)):
            ...
            if "boxes" in name:
                ...
                raise ConverterError(get_error_message('ERROR_TF_CAFFE_SSD_UNKNOWN_INPUT')(descriptor.layer_name, name))

        ssd_detection_input_names = [ssd_boxes_input, ssd_classes_input]
        if ssd_boxes_input == "" or ssd_classes_input == "":
            raise ConverterError(get_error_message('ERROR_TF_CAFFE_SSD_REQUIRES_2_INPUTS')(descriptor.layer_name))
