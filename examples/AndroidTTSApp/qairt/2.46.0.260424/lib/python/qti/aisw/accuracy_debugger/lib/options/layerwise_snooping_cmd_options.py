...
class LayerwiseSnoopingCmdOptions(CmdOptions):
    ...
    def initialize(self):
        ...
        optional.add_argument(
            ...
            help="additional context binary generator arguments in a string. \
        ...
        optional.add_argument('-o', '--output_tensor', type=str, required=False, action='append',
    ...
    def verify_update_parsed_args(self, parsed_args):
        ...
        if parsed_args.runtime == "dsp":
            dspArchs = [
                r.value for r in Runtime if r.value.startswith("dsp") and r.value not in ["dsp"]
            ]
        ...
        if not ((linux_target and linux_arch) or (android_target and android_arch) or
                (x86_64_windows_msvc_target and x86_64_windows_msvc_arch) or
                (wos_target and wos_arch)) and arch != "aarch64-qnx":
            raise ParameterError("Target device and architecture mismatch.")

        parsed_args.step_size = int(parsed_args.step_size)
        if parsed_args.step_size > 1 and (parsed_args.add_layer_outputs
                                          or parsed_args.add_layer_types
                                          or parsed_args.skip_layer_types
                                          or parsed_args.skip_layer_outputs
                                          or parsed_args.start_layer or parsed_args.end_layer):
            raise ParameterError(
                f'--step_size (> 1) should not be used along with --add_layer_outputs, \
