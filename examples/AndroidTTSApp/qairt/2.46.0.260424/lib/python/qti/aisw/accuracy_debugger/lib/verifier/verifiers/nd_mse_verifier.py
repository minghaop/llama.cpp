...
class MSEVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            ...
            return VerifyResult(layer_type, len(inference_raw), tensor_dimensions, MSEVerifier.NAME, msg, "")

        delta = golden_raw - inference_raw
        mse = np.mean(delta**2)
        # Handle nan value
        mse = 'NaN' if np.isnan(mse) else mse
        ...
        return VerifyResult(layer_type, len(inference_raw), tensor_dimensions, MSEVerifier.NAME, mse,
                            MSEVerifier.UNITS)
