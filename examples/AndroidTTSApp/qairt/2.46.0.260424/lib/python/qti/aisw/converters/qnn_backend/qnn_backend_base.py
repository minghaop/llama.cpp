...
class QnnConverterBackendBase(ConverterBackend):
    class ArgParser(ConverterBackend.ArgParser):
        def __init__(self):
            ...
            self.add_optional_argument('--float_bitwidth', type=str, default='32',
                                       help='Use the --float_bitwidth option to convert the graph to the specified float'
                                       ' bitwidth, either 32 (default), 16, or bf16.')
            self.add_optional_argument('--float_bw', type=str, default='32',
    ...
    def __init__(self, args):
        ...
        # Process float_bitwidth argument
        float_bitwidth_str = args.float_bitwidth
        if "--float_bitwidth" not in sys.argv and "--float_bw" in sys.argv:
            float_bitwidth_str = str(args.float_bw)

        # Parse float_bitwidth and set isBfloat16 flag
        if float_bitwidth_str == 'bf16':
            self.float_bitwidth = 16
            self.isBfloat16 = True
        elif float_bitwidth_str in ['16', '32']:
            self.float_bitwidth = int(float_bitwidth_str)
            self.isBfloat16 = False
        else:
            raise ValueError("Invalid float_bitwidth value: '{}'. Must be '16', '32', or 'bf16'.".format(float_bitwidth_str))
        ...
        # Will be used to indicate whether QAIRT Converter's Float Fallback will be triggered later
        self.qairt_converter_float_fallback = False
        ...
        self.process_custom_io_in_qnn_backend = True
    ...
    def process_float_datatype(self, is_bias=False):
        ...
        if (self.is_online_construction or self.float_bitwidth == 32 or
           (self.float_bias_bitwidth == 32 and is_bias) or self.qairt_converter_float_fallback):
            return ir_graph.QNN_DATATYPE_FLOAT_32
        elif self.float_bitwidth == 16 and self.isBfloat16:
            return ir_graph.QNN_DATATYPE_BFLOAT_16
    ...
    def is_float(data_type):
        if ((data_type == ir_graph.QNN_DATATYPE_FLOAT_32) or
                (data_type == ir_graph.QNN_DATATYPE_FLOAT_16) or
                (data_type == ir_graph.QNN_DATATYPE_BFLOAT_16)):
            return True
        else:
            return False
    ...
    def create_tensor_info(
        ...
        if quantizable and self.is_float(tensor_data_type):
            ...
            if tensor_data_type == ir_graph.QNN_DATATYPE_BFLOAT_16 and data is not None:
                #Bfloat16 conversion from fp32 to bfloat16
                data = self.c_utils.buffer_fp32_to_bf16(data.flatten())
    ...
    def get_output_info(self, graph, tensor_name, tensor_type=qnn_definitions.QNN_TENSOR_TYPE_NATIVE, tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32,
        ...
        if self.input_network.split(".")[-1]!='onnx' and not 'REMOVE_DISCONNECTED_NODES' in self.optimizationPassManager.get_cpp_ir_pass_list():
        ...
        if self.process_custom_io_in_qnn_backend:
    ...
    def get_outputs_info(self, node, graph, tensor_data_type=None, check_encodings=True, original_output=None):
        ...
        if tensor_data_type is None:
            ...
            # We have already inferred output dtype for the tensors in py-ir graph.
            # Use the dtype info here
            output_buf = graph.get_buffer(node.output_names[0])
            tensor_data_type = output_buf.dtype
        # If QAIRT Converter's Float Fallback is about to trigger later, then reset the Python Graph's float
        # tensor's datatype back to Float32, as Float bitwidth will be handled during Float Fallback step
        if self.qairt_converter_float_fallback:
            if tensor_data_type == ir_graph.Qnn_DataType_t.QNN_DATATYPE_FLOAT_16:
                tensor_data_type = ir_graph.Qnn_DataType_t.QNN_DATATYPE_FLOAT_32
                # If tensor datatype is FP16, reset to FP32 and set check_encodings (quantizable) back to True
                check_encodings = True

        if tensor_data_type not in [ir_graph.Qnn_DataType_t.QNN_DATATYPE_FLOAT_32,
                                    ir_graph.Qnn_DataType_t.QNN_DATATYPE_FLOAT_16] and \
                tensor_data_type not in qnn_quantized_types:
    ...
    def get_ir_graph(self, graph):
        ...
        try:
            ...
            if graph.quantization_params:
                ...
                if self.qairt_converter and self.args.export_format == ExportFormatType.DLC_DEFAULT:
                    self.qairt_converter_float_fallback = any(q['output_encodings'] or q['param_encodings']
                                                              for q in graph.quantization_params.values())

            # @if AISW_ENABLE_IRGRAPH_TRANSFORMS
            if 'PROCESS_CUSTOM_IO_QUANT_INFO' in OptimizationPassModeManager.get_instance().get_cpp_ir_pass_list():
                self.process_custom_io_in_qnn_backend = False
            ...
            # Set Target backend name
            if self.backend_name is not None:
                self.model.set_backend_name(self.backend_name)

            ir_graph = self.model.get_ir_graph()
            self.external_data_path = graph.external_data_path
        except BaseException as e:
            raise e

        # Enable optional bias for quantize version 2 mode
        # This allows operations like Conv/FC to work without bias during G2G optimization
        external_quant_params = graph.user_quantization_overrides
        use_optional_bias = (external_quant_params and
                             "version" in external_quant_params and
                             external_quant_params["version"] == "2.0.0" or
                             graph.use_quantize_v2)

        # Set optional bias flag in C-IR graph to control bias handling during optimization passes
        ir_graph.set_optional_bias(use_optional_bias)
            ...
            for idx, input_name in enumerate(node.input_names):
                ...
                if input_tensor is not None and not input_tensor.is_static():
                    ...
                    if input_tensor_data_type != output_tensor_data_types[0]:
                        ...
                        convert_op = op_adapter.ConvertOp(convert_name, to_type=output_tensor_data_types[0])
    ...
    # In case of defer loading, Onnx model is copied/dumped into temp directory
    # after onnx simplification. It should only be deleted after conversion is complete
    # as weights might not be loaded before serialization
    def __del__(self):
        if self.external_data_path:
            if os.path.isdir(self.external_data_path):
                shutil.rmtree(self.external_data_path)
            elif os.path.isfile(self.external_data_path):
                os.remove(self.external_data_path)
