...
import numpy as np
from qti.aisw.converters.common.converter_ir.op_adapter import RolledLstmOp, LstmOp, ReshapeOp, ConstantOp
...
from qti.aisw.converters.common.graph_optimizer import OptimizationPassModeManager
...
class LstmLayerResolver(LayerResolver):
    class Descriptor(LayerDescriptor):
        ...
        def is_unrolled_cell_of(self, lstm_descriptor):
            ...
            raise ConverterError(code_to_message.get_error_message('ERROR_TF_GENERAL_ABSTRACT_CLASS_MUST_BE_INHERITED')
                                 ("is_unrolled_cell_of", str(self.__class__.__name__)))
        ...
        def resolve_weights(self, graph_helper, state_shape, **kwargs):
            ...
            """
            raise ConverterError(code_to_message.get_error_message('ERROR_TF_GENERAL_ABSTRACT_CLASS_MUST_BE_INHERITED')
                                 ("resolve_weights", str(self.__class__.__name__)))
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        """
        raise ConverterError(code_to_message.get_error_message('ERROR_TF_GENERAL_ABSTRACT_CLASS_MUST_BE_INHERITED')
                             ("resolve_layer", str(self.__class__.__name__)))
    ...
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if not is_stacked_above_cell:
            ...
            if len(list(cell_input_descriptors)) != 1:
                raise ConverterError('Unable to resolve LSTM {} input layer name.'.format(
        ...
        if cell_state_descriptors:
            ...
            if descriptor.returns_state() and not is_stacked_below_cell:
                ...
                user_initial_state = all(d for d in cell_state_descriptors
        ...
        if not user_initial_state:
            ...
            output_names = [descriptor.rolled_cell_output_name, *descriptor._output_tensor_names]
        ...
        rolled_lstm_name = descriptor.cell_0.child_ops[-1].name
        input_weights_name = rolled_lstm_name + '_input_weights'
        hidden_state_weights_name = rolled_lstm_name + '_hidden_state_weights'
        gate_biases_name = rolled_lstm_name + '_gate_biases'
        norm_weights_name, cell_state_weights_name = '', ''
        proj_weights_name = rolled_lstm_name + '_proj_w' if descriptor.proj_weights else ''
        proj_bias_name = rolled_lstm_name + '_proj_b' if descriptor.proj_biases else ''
        ...
        # If qairt_converter flag is set on the IR graph, build a stateful LSTMOp with per-gate inputs.
        if getattr(ir_graph, 'qairt_converter', False) and 'MULTI_TIME_STEPS_LSTM' in OptimizationPassModeManager.get_instance().get_cpp_ir_pass_list():
            # Derive per-gate parameter names following the same convention as ONNX qairt
            input_w_to_forget_gate_name = input_weights_name + '_input_w_to_forget_gate'
            input_w_to_cell_gate_name   = input_weights_name + '_input_w_to_cell_gate'
            input_w_to_output_gate_name = input_weights_name + '_input_w_to_output_gate'
            input_w_to_input_gate_name  = input_weights_name + '_input_w_to_input_gate'

            rec_w_to_forget_gate_name = hidden_state_weights_name + '_recurrent_w_to_forget_gate'
            rec_w_to_cell_gate_name   = hidden_state_weights_name + '_recurrent_w_to_cell_gate'
            rec_w_to_output_gate_name = hidden_state_weights_name + '_recurrent_w_to_output_gate'
            rec_w_to_input_gate_name  = hidden_state_weights_name + '_recurrent_w_to_input_gate'

            b_to_forget_gate_name = gate_biases_name + '_b_to_forget_gate'
            b_to_cell_gate_name   = gate_biases_name + '_b_to_cell_gate'
            b_to_output_gate_name = gate_biases_name + '_b_to_output_gate'
            b_to_input_gate_name  = gate_biases_name + '_b_to_input_gate'

            # Construct input list in LstmOp expected order. Leave optional norm/cell weights empty if not present.
            input_names = [
                input_layer_name,                   # 0: data input
                input_w_to_forget_gate_name,        # 1
                input_w_to_cell_gate_name,          # 2
                input_w_to_output_gate_name,        # 3
                rec_w_to_forget_gate_name,          # 4
                rec_w_to_cell_gate_name,            # 5
                rec_w_to_output_gate_name,          # 6
                b_to_forget_gate_name,              # 7
                b_to_cell_gate_name,                # 8
                b_to_output_gate_name,              # 9
                h_0_input_name,                     # 10
                c_0_input_name,                     # 11
                '',                                 # 12 norm_w_to_input_gate
                '',                                 # 13 norm_w_to_forget_gate
                '',                                 # 14 norm_w_to_cell_gate
                '',                                 # 15 norm_w_to_output_gate
                input_w_to_input_gate_name,         # 16
                rec_w_to_input_gate_name,           # 17
                '',                                 # 18 cell_w_to_input_gate
                '',                                 # 19 cell_w_to_forget_gate
                '',                                 # 20 cell_w_to_output_gate
                b_to_input_gate_name,               # 21
                proj_weights_name,                  # 22
                proj_bias_name,                     # 23
                ''                                  # 24
            ]

            # Prepare per-gate tensors from merged weights/biases (already in IFOG order)
            # Input weights: [input_size, 4*hidden_size] -> split into 4 [input_size, hidden_size], then transpose to [hidden_size, input_size]
            input_w_splits = np.split(input_weights, indices_or_sections=4, axis=1)
            iw_I = np.ascontiguousarray(input_w_splits[0].transpose(), dtype=np.float32)
            iw_F = np.ascontiguousarray(input_w_splits[1].transpose(), dtype=np.float32)
            iw_O = np.ascontiguousarray(input_w_splits[2].transpose(), dtype=np.float32)
            iw_G = np.ascontiguousarray(input_w_splits[3].transpose(), dtype=np.float32)

            # Recurrent weights: [hidden_size, 4*hidden_size] -> split into 4 [hidden_size, hidden_size]
            rec_w_splits = np.split(gates_weights, indices_or_sections=4, axis=1)
            rw_I = np.ascontiguousarray(rec_w_splits[0].transpose(), dtype=np.float32)
            rw_F = np.ascontiguousarray(rec_w_splits[1].transpose(), dtype=np.float32)
            rw_O = np.ascontiguousarray(rec_w_splits[2].transpose(), dtype=np.float32)
            rw_G = np.ascontiguousarray(rec_w_splits[3].transpose(), dtype=np.float32)

            # Biases: [4*hidden_size] -> split into 4 [hidden_size]
            bias_splits = np.split(gates_biases, indices_or_sections=4, axis=0)
            b_I = np.ascontiguousarray(bias_splits[0], dtype=np.float32)
            b_F = np.ascontiguousarray(bias_splits[1], dtype=np.float32)
            b_O = np.ascontiguousarray(bias_splits[2], dtype=np.float32)
            b_G = np.ascontiguousarray(bias_splits[3], dtype=np.float32)

            params_dict = {
                input_w_to_forget_gate_name: iw_F,
                input_w_to_cell_gate_name:   iw_G,
                input_w_to_output_gate_name: iw_O,
                input_w_to_input_gate_name:  iw_I,
                rec_w_to_forget_gate_name:   rw_F,
                rec_w_to_cell_gate_name:     rw_G,
                rec_w_to_output_gate_name:   rw_O,
                rec_w_to_input_gate_name:    rw_I,
                b_to_forget_gate_name:       b_F,
                b_to_cell_gate_name:         b_G,
                b_to_output_gate_name:       b_O,
                b_to_input_gate_name:        b_I
            }
            # Optional projection weights and bias
            if proj_weights_name:
                params_dict[proj_weights_name] = descriptor.proj_weights
            if proj_bias_name:
                params_dict[proj_bias_name] = descriptor.proj_biases

            self.prepare_params_as_constants(ir_graph, params_dict, rolled_lstm_name)

            return ir_graph.add(LstmOp(name=rolled_lstm_name,
                                       time_major=False,
            ...
            # Default path: build RolledLstmOp with merged weights/biases
            input_names.extend([input_weights_name, hidden_state_weights_name, gate_biases_name,
                                norm_weights_name, cell_state_weights_name, proj_weights_name, proj_bias_name])

            params_dict = {}
            # The weights should be in correct format defined in RolledLstmOp.hpp, which leads to adding the transpose here
            params_dict[input_weights_name] = np.ascontiguousarray(input_weights.transpose(), dtype=np.float32)
            params_dict[hidden_state_weights_name] = np.ascontiguousarray(gates_weights.transpose(), dtype=np.float32)
            params_dict[gate_biases_name] = gates_biases
            # Projection weights and bias are the optional inputs that can be captured in descriptor
            if proj_weights_name:
                params_dict[proj_weights_name] = descriptor.proj_weights
            if proj_bias_name:
                params_dict[proj_bias_name] = descriptor.proj_biases
            ...
            return ir_graph.add(RolledLstmOp(name=rolled_lstm_name,
                                             time_major=False,
                                             reset_state_at_time_step_0=reset_at_time_step_0,
                                             c_0_input_name=c_0_input_name,
                                             h_0_input_name=h_0_input_name,
