        ...
        try:
            ...
            try:
                Visualizers.histogram_visualizer(golden_data, target_data,
                                                 os.path.join(output_dir, "Histograms.html"))
                Visualizers.diff_visualizer(golden_data, target_data,
                                            os.path.join(output_dir, "Diff_plots.html"))
                Visualizers.cdf_visualizer(golden_data, target_data,
                                           os.path.join(output_dir, "CDF_plots.html"))
            except Exception as e:
                self._logger.error(
                    f"Plots generation failed for {filename} with error {e}, proceeding to next tensor.")
            ...
            # Handle nan values
            for key, value in tensor_ranges.items():
                result[key] = 'NaN' if np.isnan(value) else value
