...
class RtolAtolVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_output) != len(inference_output):
            msg = get_message("ERROR_VERIFIER_SIZE_MISMATCH")(str(len(golden_output)),
                                                                 (str(len(inference_output))))
            return VerifyResult(layer_type, len(inference_output), tensor_dimensions, self._get_verifier_name(), msg, "")
        ...
        # Handle nan value
        percent_not_close = 'NaN' if np.isnan(percent_not_close) else percent_not_close
        ...
        return VerifyResult(layer_type, len(inference_output), tensor_dimensions,
