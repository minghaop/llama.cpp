...
from qti.aisw.converters.common.converter_ir.op_adapter import Pool2dOp, IRPaddingStrategies
...
class PoolingLayerBuilder(LayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        return ir_graph.add(Pool2dOp(name=descriptor.layer_name,
    ...
    @classmethod
    def calculate_padding(cls, padding_size_strategy, input_size, strides, pool_dims, layer_name):
        ...
        if padding_size_strategy.decode() == 'SAME':
            ...
            ir_padding_strategy = ir_graph.PADDING_SIZE_IMPLICIT_SAME_END
        elif padding_size_strategy.decode() == 'VALID':
            pads = [[0, 0], [0, 0]]
            ir_padding_strategy = ir_graph.PADDING_SIZE_IMPLICIT_VALID
        else:
            raise ValueError("Unsupported TF padding strategy {} for pool layer {}".format(
                padding_size_strategy.decode(), layer_name))
...
class AvgPoolingLayerResolver(PoolingLayerResolver):
    ...
    def __init__(self):
        super(AvgPoolingLayerResolver, self).__init__('AvgPooling', AvgPoolingLayerResolver.Descriptor,
                                                      ir_graph.QNN_OP_POOL_AVG_2D, 'AvgPool')
...
class MaxPoolingLayerResolver(PoolingLayerResolver):
    ...
    def __init__(self):
        super(MaxPoolingLayerResolver, self).__init__('MaxPooling', MaxPoolingLayerResolver.Descriptor,
                                                      ir_graph.QNN_OP_POOL_MAX_2D, 'MaxPool')
