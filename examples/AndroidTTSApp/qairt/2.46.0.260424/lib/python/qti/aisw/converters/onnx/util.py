...
# TensorProto.INT4/UINT4 were added in ONNX 1.15 (opset 21).
# Define fallbacks so the module can be imported with older ONNX versions.
TENSORPROTO_INT4 = getattr(TensorProto, 'INT4', 22) if onnx else 22
TENSORPROTO_UINT4 = getattr(TensorProto, 'UINT4', 21) if onnx else 21
...
# numpy has no native int4/uint4, map to closest types for dtype tracking
if TENSORPROTO_INT4 is not None:
    onnx_to_np_dtype[TENSORPROTO_INT4] = np.dtype("int8")
if TENSORPROTO_UINT4 is not None:
    onnx_to_np_dtype[TENSORPROTO_UINT4] = np.dtype("uint8")
...
def decode_int4_raw_data(raw_data_bytes, signed=True):
    int4_values = []
    for byte in raw_data_bytes:
        # ONNX spec: first element in LSB nibble, second element in MSB nibble
        low = byte & 0x0F
        high = (byte >> 4) & 0x0F
        values = [low, high]

        if signed:
            values = [v - 16 if v >= 8 else v for v in values]

        int4_values.extend(values)
    return int4_values
...
def decode_tensorproto_raw_data(tensor_proto):
    ...
    if dtype == TENSORPROTO_INT4:
        values = decode_int4_raw_data(raw_data, signed=True)
        # from_array packs two INT4 values per byte, so odd-element tensors
        # (including scalars) produce one extra padding value from the high nibble.
        num_elements = product(tensor_proto.dims)
        return np.array(values[:num_elements], dtype=np.int8)
    elif dtype == TENSORPROTO_UINT4:
        values = decode_int4_raw_data(raw_data, signed=False)
        num_elements = product(tensor_proto.dims)
        return np.array(values[:num_elements], dtype=np.uint8)
    else:
        np_dtype = onnx_tensor_dtype_to_np_dtype(dtype)
        return np.frombuffer(raw_data, dtype=np_dtype)


def get_quantized_range(zp, bw):
    """Compute quantized min/max range for a given zero-point dtype and bitwidth."""
    if bw == 4:
        if str(zp.dtype).startswith('int'):
            return -8, 7
        else:
            return 0, 15
...
"""
Extracts quantization information from a zero-point tensor.

Args:
    zp: The zero-point tensor as a TensorProto or NumPy array.

Returns:
    A tuple containing the following information:
        - is_symmetric: Whether the quantization is symmetric (i.e., zero-point is close to 0).
        - bw: The bitwidth of the quantization.
        - offset: The offset value for the quantization.
        - dtype: The data type of the quantization as a string.
        - zp: The zero-point tensor decoded to a NumPy array.

Raises:
    ValueError: If the zero-point data type is not supported.
"""
def get_quant_info(zp):
    dtype = None
    if isinstance(zp, onnx.onnx_ml_pb2.TensorProto):
        if zp.data_type == TENSORPROTO_INT4:
            dtype = "int4"
        elif zp.data_type == TENSORPROTO_UINT4:
            dtype = "uint4"
    ...
    if dtype not in dtype_to_bitwidth:
        raise ValueError(f"Unsupported zero point data type: {dtype}")
...
"""
Creates an encoding dictionary for a quantized tensor.

Args:
    name (str): The name of the tensor.
    scale (float): The scale factor for the quantization.
    zp: The zero-point tensor as a TensorProto or NumPy array.
    axis (int, optional): The axis along which the quantization is applied. Defaults to 1.
    block_size (int, optional): The block size for the quantization. Defaults to 0.

Returns:
    A tuple containing the encoding dictionary and the decoded zero-point tensor.
    The encoding dictionary has the following keys:
        - name: The name of the tensor.
        - bw: The bitwidth of the quantization.
        - min: The minimum value of the quantized range.
        - max: The maximum value of the quantized range.
        - scale: The scale factor for the quantization.
        - offset: The offset value for the quantization.
        - is_symmetric: Whether the quantization is symmetric (i.e., zero-point is close to 0).
        - overridden: Whether the encoding has been overridden.
        - axis: The axis along which the quantization is applied.
        - block_size: The block size for the quantization.
        - dtype: The data type of the quantization as a string.

"""
def get_encoding(name, scale, zp, axis=1, block_size=0):
    is_symmetric, bw, offset, dtype, zp = get_quant_info(zp)
    qmin, qmax = get_quantized_range(zp, bw)
...
class WeightProvider(object):
    ...
    def set_weight_map(self,model,remove_model_weights=True, lora_tensor_names=[]):
        ...
        while count < total_weights:
            ...
            dtype = tensor_proto.data_type
            if dtype == TENSORPROTO_INT4 or dtype == TENSORPROTO_UINT4 or not _is_recognized_tensor_dtype(dtype):
                # Store TensorProto directly for INT4/UINT4 (numpy has no native
                # 4-bit dtype) and for data types not recognised by the current
                # ONNX version (e.g. INT4/UINT4 on ONNX < 1.15, FLOAT8 on
                # ONNX < 1.14).  Decoding is deferred to point of use.
                self.weight_map[tensor_name] = WeightData(tensor_proto, False, tensor_proto.dims, dtype)
                count += 1
            else:
    ...
    def set_lazy_weight_map(self,model, lora_tensor_names=[]):
        ...
        while count < total_weights:
            ...
            if not dims:
                ...
                was_scalar = True
                ir_constant_tensor = ir_graph.PyIrConstantTensor(onnx_np_tensor)
            elif tensor_proto.data_type == TENSORPROTO_INT4 or tensor_proto.data_type == TENSORPROTO_UINT4 or not _is_recognized_tensor_dtype(tensor_proto.data_type):
                # Store TensorProto directly for INT4/UINT4 (numpy has no native
                # 4-bit dtype) and for data types not recognised by the current
                # ONNX version (e.g. INT4/UINT4 on ONNX < 1.15, FLOAT8 on
                # ONNX < 1.14).  Decoding is deferred to point of use.
                self.weight_map[tensor_name] = WeightData(tensor_proto, False, tensor_proto.dims,
    ...
    def insert(self, key, weights, was_scalar=False, transform_manager=None):
        ...
        if not isinstance(weights, ir_graph.PyIrConstantTensor):
            ...
            dtype = ir_constant_tensor.data_type
...
def _is_recognized_tensor_dtype(data_type: int) -> bool:
    """
    Check whether the given TensorProto data_type value is recognised by the
    installed ONNX version.  Returns False for types introduced in newer ONNX
    releases (e.g. FLOAT8 variants in 1.14, INT4/UINT4 in 1.15) when running
    with an older ONNX package.
    """
    try:
        onnx_tensor_dtype_to_np_dtype(data_type)
        return True
...
def convert_data_to_raw_data(model: ModelProto, include_attributes: bool) -> None:
    ...
    def get_new_tensor(tensor):
        # Skip INT4/UINT4 tensors (sub-byte types always use raw_data and
        # numpy has no native 4-bit dtype) and tensors whose data type is
        # not recognised by the current ONNX version (e.g. INT4/UINT4 on
        # ONNX < 1.15, FLOAT8 types on ONNX < 1.14).
        if tensor.data_type == TENSORPROTO_INT4 or tensor.data_type == TENSORPROTO_UINT4 or not _is_recognized_tensor_dtype(tensor.data_type):
            return tensor
        tensor_data = numpy_helper.to_array(tensor)
    ...
    for graph in get_graphs(model):
        for idx, tensor in enumerate(graph.initializer):
            # Skip INT4/UINT4 (sub-byte, no native numpy dtype) and tensors
            # whose data type is not recognised by the current ONNX version.
            if tensor.data_type == TENSORPROTO_INT4 or tensor.data_type == TENSORPROTO_UINT4 or not _is_recognized_tensor_dtype(tensor.data_type):
                continue
