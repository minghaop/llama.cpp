...
class RelayQuantization(object):
    ...
    def get_quantization_params(op_name: str,
                                ...
                                relay_params: dict,
                                attrs: dict,
                                use_quantize_v2: bool):
        if len(input_names) < 2:
            raise ValueError("Missing quantization params for op {}. Should contain [input, scale, offset(optional)] but got {}"
        ...
        if "dtype" not in attrs:
            raise ValueError("Missing quantization dtype attrs for op {}".format(op_name))
        if attrs['dtype'] not in RelayQuantization.QuantTypes:
            raise ValueError("Unsupported quantize dtype {} for op {}, only support int8/uint8/int16/uint16"
                             .format(attrs['dtype'], op_name))
        dtype = attrs['dtype']
        np_dtype = RelayQuantization.QuantTypes[dtype]
        ...
        if len(input_names) > 2:
            zero_point = relay_params[input_names[2]]
            log_debug3('Found quantization zero_point {} of type {}'.format(zero_point, type(zero_point)))
            if isinstance(zero_point, tvm.runtime.ndarray.NDArray) or isinstance(zero_point, tvm.runtime.NDArray):
                zero_point = zero_point.asnumpy().astype(np.int32)

        if dtype and dtype.startswith("int"):
            is_symmetric = np.allclose(zero_point, 0)
        else:
            is_symmetric = False

        # We need to calculate the min/max before shifting zero_point
        min = np.float32(np.iinfo(np_dtype).min - zero_point) * scale
        max = np.float32(np.iinfo(np_dtype).max - zero_point) * scale

        if dtype and dtype.startswith("int") and not use_quantize_v2:
            # activation is quantized to unsigned integer in SNPE/QNN v1,
            # so we need to shift offset for signed integer
            zero_point = zero_point + 2 ** (bw - 1)

        q_params = {
            'name': op_name,
            'scale' : scale,
            'offset': -zero_point,  # Note: offset is negative of zero_point
            "min": min,
            "max": max,
            'dtype': dtype,
            "bitwidth": bw,
