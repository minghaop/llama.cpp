...
class Model(object):
    ...
    class Input(object):
        ...
        def __init__(self, name: str, shape: List[str], batch: int):
            """
    ...
    def __load(
        ...
        inferred_model_input_names = set(
            [util.remove_suffix(name) for name in util.get_input_info(graph).keys()]
        )
        inferred_model_output_names = set(
            [util.remove_suffix(name) for name in util.get_output_info(graph).keys()]
        )
        input_tensor_info = {}
        with self.session.graph.as_default():
            ...
            for name, shape in zip(input_nodes_names, input_nodes_shapes):
                ...
                inputs.append(Model.Input(name, batched_shape, batch))
...
class ModelLoader(FrameworkModelLoader):
    ...
    def const_op_types(self) -> List[str]:
        """
        Get a list of constant operators types provided by tensorflow.

        :return List[str]: List of tensorflow operator types.
        """
        return ["Const"]

    def get_op_type(self, node: NODE_DEF) -> str:
        """
        Get the op type of a given node.

        :param NODE_DEF node: Tensorflow node reference.
    ...
    def get_input_nodes_of_graph(self) -> List[NODE_DEF]:
        """
        Get the list of input nodes of tensorflow graph.

        :return List[NODE_DEF]: List of input nodes.
        """
        input_tensors = [util.remove_suffix(name) for name in self.get_input_names()]
        dangling_input_tensors = [
            util.remove_suffix(name) for name in self.get_input_names()
        ]
    ...
    def get_output_nodes_of_graph(self) -> List[NODE_DEF]:
        """
        Get the list of output nodes of tensorflow graph.

        :return List[NODE_DEF]: List of output nodes.
        """
        node_by_output_name = util.get_node_by_output_name(self.model.session.graph)
        output_tensors = self.get_output_names()
        dangling_output_tensors = self.get_output_names()

        output_nodes = []
        for tensor_name in output_tensors:
            if tensor_name in node_by_output_name:
                node = node_by_output_name[tensor_name]
                output_nodes.append(node)
                dangling_output_tensors.remove(tensor_name)
    ...
    def get_input_op_types(self, node: NODE_DEF) -> List[str]:
        """
        ...
        return [
            (
                "Input"
                if self.get_op_type(node) in ["Placeholder", "PlaceholderWithDefault"]
                else self.get_op_type(node)
            )
            for node in parent_nodes
        ]

    def get_output_op_types(self, node: NODE_DEF) -> List[str]:
        """
        ...
        if self.is_output_node(node):
            return ["Output" for _ in self.get_node_outputs(node)]
    ...
    def get_input_info(self) -> Dict[str, TensorInfo]:
        ...
        :return Dict[str, TensorInfo]: Dict mapping of tensor name to its
            TensorInfo object.
        """
        return util.get_input_info(self.model.session.graph)
    ...
    def get_output_info(self) -> Dict[str, TensorInfo]:
        ...
        :return Dict[str, TensorInfo]: Dict mapping of tensor name to its
            TensorInfo object.
        """
        return util.get_output_info(self.model.session.graph)

    def get_custom_io_container(self):
        pass

    def get_parent_nodes(self, node: NODE_DEF) -> List[NODE_DEF]:
        """
        Get the list of all the parent nodes of the given node name.

        :param NODE_DEF node: Reference node whose parent nodes are to be found.
        :return List[NODE_DEF]: List of parent nodes.
        """
        return util.get_parent_nodes(self.model.session.graph, node)

    def get_children_nodes(self, node: NODE_DEF) -> List[NODE_DEF]:
        """
        Get the list of all the child nodes of the given node name.

        :param NODE_DEF node: Reference node whose children nodes are to be found.
        :return List[NODE_DEF]: List of children nodes.
        """
        return util.get_children_nodes(self.model.session.graph, node)

    def get_node_by_name(self, node_name: str) -> Union[None, NODE_DEF]:
        """
        Get the node by given name from the graph.

        :param str node_name: Node name.
        :return NODE_DEF: Node reference if found else None.
        """
        for node in self.get_nodes():
    ...
    def get_node_inputs(self, node: NODE_DEF) -> List[tf.Tensor]:
        """
        Get the input tensor of a given node.

        :param NODE_DEF node: Reference tensorflow node.
        :return List[tf.Tensor]: List of input tensors of given node.
        """
        return util.get_node_inputs(node, self.model.session.graph)

    def get_node_outputs(self, node: NODE_DEF) -> List[tf.Tensor]:
        """
        Get the output tensor of a given node.

        :param NODE_DEF node: Reference tensorflow node.
        :return List[tf.Tensor]: List of output tensors of given node.
        """
        return util.get_node_outputs(node, self.model.session.graph)

    def get_supported_operators(self) -> List[str]:
        """
        Get the list of all the operators operators by tensorflow.
        Note: There is no direct mechanism to get the list of all the operators
              which are supported by tensorflow. Hence we are returning list of
              all the operators present in the given model.

        :return List[str]: List of supported operators.
        """
        all_ops = set()
        for node in self.get_nodes():
            all_ops.add(self.get_op_type(node))
        return list(all_ops)
    ...
    def summarize_model(self) -> FrameworkSummary:
        ...
        graph = self.model.session.graph
        total_params = util.get_model_params(self._model.graph_def)
        ops_counter = util.get_unique_ops(graph)

        input_specs_dict = {}
        output_specs_dict = {}
        for input_name, input_tensor_info in util.get_input_info(graph).items():
            input_specs_dict[input_name] = (
                input_tensor_info.shape,
                "input",
                input_name,
            )
        for output_name, output_tensor_info in util.get_output_info(graph).items():
