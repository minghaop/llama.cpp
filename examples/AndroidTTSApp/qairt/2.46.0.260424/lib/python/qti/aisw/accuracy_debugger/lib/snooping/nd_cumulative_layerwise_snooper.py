...
from qti.aisw.accuracy_debugger.lib.utils.nd_constants import Framework, DataType
...
class CumulativeLayerwiseSnooping(Snooper):
    ...
    def run(self, model=None, profile_info=None):
        ...
        if self.quantization_overrides is None:
            self.logger.debug('Extracting encodings from model_net.json')
            self.quantization_overrides = self.extract_and_filter_encodings(self.output_dir, model)
        ...
        while (True):
            ...
            if profile_info and s_cur_layer_out_name in profile_info:
                ...
                # Set default datatype (fp32) when a layer details are not recorded in profile_info
                layer_dtype_map[s_cur_layer_out_name] = DataType.DEFAULT_OUTPUTS_DATATYPE.value
            ...
            if not skip_compare:
                ...
                if not self.disable_layout_transform:
                ...
                for elem in original_outputs:
                    ...
                    qnn_model_json_path = os.path.join(self.output_dir, 'inference_engine',
                                                       s_cur_layer_out_name, 'model_net.json')
                    if original_output_name not in self.is_transpose_needed_dict and \
                                                    not self.disable_layout_transform:
                        # Permute target tensor to align with golden tensor
        ...
        for verifier_name in overall_comp_list:
            plots_data[f'{verifier_name}(current_layer)'] = [
                df.index.values, df[verifier_name].values
            ]

        # Gather original outputs scores
        for layer_name, data in zip(df.index.values, df['Orig Outputs'].values):
            try:
                for output_name in data.keys():
                    for comparator, score in data[output_name].items():
                        ...
                        if plot_name not in plots_data.keys():
                            plots_data[plot_name] = [[layer_name], [score]]
                        else:
                            plots_data[plot_name][0].append(layer_name)
                            plots_data[plot_name][1].append(score)
            except Exception as e:
                self.logger.warning(
                    f'Skipping Orig outputs verifier data for {layer_name} while plotting.')

        plots_save_dir = os.path.join(self.output_dir, 'plots')
        os.makedirs(plots_save_dir, exist_ok=True)
        for verifier_name, plot_values in plots_data.items():
            layer_names, scores = plot_values
            try:
                self.logger.debug(f'Plotting graph for {verifier_name} scores...')
                Visualizers.line_plot(x=layer_names, y=scores, plot_name=verifier_name,
