...
class InferenceEngineManager(object):
    ...
    def run_inference_engine(self, netrun_lock: multiprocessing.synchronize.Lock = None):
        if self.inference_engine.engine_type ==  Engine.QAIRT.value:
            self.inference_engine.run(netrun_lock)
        else:
            self.inference_engine.run()
        if self.inference_engine.engine_type in [
                Engine.SNPE.value, Engine.QNN.value, Engine.QAIRT.value
        ]:
            try:
