...
class CustomOnnxOp(CustomOp):
        ...
        for param_info in param_infos:
            ...
            if not len(result):
                if param_info.default_value is not None:
                    ...
                    raise KeyError("Required custom op attribute: {} is not present in {} op: {}".
                                   format(param_info.name, src_op.op_type, src_op.name))
    ...
    def infer_output_shapes(self, node, model=None, perform_shape_inference=False):
        ...
        if model is None:
            return output_dims
        if not model:
            raise RuntimeError("Please provide model that shape should be inferred from for node {}".
                               format(node.op.name))
    ...
    def set_tensor_data_types(self, node):
        for output in node.output:
            ...
            if not onnx_dtype:
                ...
                onnx_op_output.data_type = convert_to_backend_type_from_numpy(onnx_tensor_dtype_to_np_dtype(onnx_dtype))
        ...
        for input_ in node.input:
            ...
            if not onnx_op_inputs:
                ...
                raise TypeError("Could not resolve datatype for custom op {} "
                                "input tensor: {} ".format(node.op.name, input_))
            ...
            if  not onnx_dtype:
                ...
                onnx_op_input.data_type = convert_to_backend_type_from_numpy(onnx_tensor_dtype_to_np_dtype(onnx_dtype))
    ...
    def validate_params(src_op, param_info):
        ...
        for param in param_info:
            if param.name not in (attr.name for attr in src_op.attribute) \
                ...
                raise KeyError("Required custom op attribute: {} is not present in {} op: {}".
                               format(param.name, src_op.op_type, src_op.name))
