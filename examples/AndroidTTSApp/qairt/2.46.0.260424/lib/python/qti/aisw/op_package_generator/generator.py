...
class QnnOpPackageGenerator:
    ...
    def parse_config(self, config_paths: List[str], output_path: Optional[str] = None, converter_op_package = False):
        ...
        for config_path in config_paths:
            ...
            op_package_collection, op_package_plugin_collection = self.__get_qnn_collection_from_op_def_collection(op_collection)
            ...
            if op_package_plugin_collection:
                self.create_package_info(op_package_collection, output_path, config_path, op_package_plugin_collection)
            else:
    ...
    def create_package_info(self, op_package_collection, output_path, config_path, op_package_plugin_collection=None):
        for package_name, package in op_package_collection.items():
            for backend, operators in package.items():
                ...
                # Determine use_qhpi from op_package_plugin_collection based on package name
                use_qhpi = op_package_plugin_collection.get(package_name, False) if op_package_plugin_collection else False
    ...
    def setup_file_paths(self, force_generation=False, gen_cmakelists=False):
        ...
        for i, package_info in enumerate(self.package_infos):
            ...
            # get templates for qnn backends - check if package has UseQHPI for QHPI generation
            use_plugin = hasattr(package_info, 'use_qhpi') and package_info.use_qhpi
            templates = QnnTemplateFileReader.get_template_type_by_backend(package_info.backend, use_plugin=use_plugin)
    ...
    def setup_backend_specific_file_paths(self, package_info, package_handler, gen_cmakelists=False):
        ...
        if package_info.backend == "CPU" or package_info.backend == "GPU":
            ...
            for makefile in ['Makefile.aarch64-oe-linux-gcc11.2', 'Makefile.aarch64-oe-linux-gcc9.3']:
                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), makefile)):
                    makefile_handler = FileHandler(makefile,
                                           make_dir_handler.dir_abs_path,
                                           copy_source_location=os.path.join(
                                                                MAKEFILE_PATH,
            ...
            if package_info.backend == "CPU":
                ...
                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), 'Makefile.qnx800-aarch64')):
                    makefile_handler = FileHandler('Makefile.qnx800-aarch64',
                                                   make_dir_handler.dir_abs_path,
                                                   copy_source_location=os.path.join(MAKEFILE_PATH,
                                                                                     package_info.backend.upper(),
                                                                                     'Makefile.qnx800-aarch64'))
                    make_dir_handler.file_handlers.extend([makefile_handler])
    ...
    def __get_qnn_collection_from_op_def_collection(cls, op_def_collection):
        ...
        """
        supported_backends = [key for key, value in op_def_collection.supported_ops.items() if value]
        # create op_collection for easy look-up
        op_package_collection = QnnPackageCollection()
        op_package_plugin_collection = {}

        # Check UseQHPI at the OpDefCollection level
        use_qhpi = op_def_collection.use_qhpi if hasattr(op_def_collection, 'use_qhpi') else False
        ...
        for backend in supported_backends:
            ...
            for op_def in backend_op_defs:
                ...
                # Populate is_quantizable from op_def_collection.op_def_dict
                for idx, inp in enumerate(resolved_op_def.input):
                    inp.data_type = []
                    inp.is_quantizable = False  # Default to False, override only if explicitly provided
                    # Override only if schema explicitly defines it
                    if resolved_op_def.type_name in op_def_collection.op_def_dict:
                        op_def_inputs = op_def_collection.op_def_dict[resolved_op_def.type_name].inputs
                        if idx < len(op_def_inputs) and hasattr(op_def_inputs[idx], 'is_quantizable'):
                            inp.is_quantizable = op_def_inputs[idx].is_quantizable

                op_package_collection[resolved_op_def.package_name] = {backend: [resolved_op_def]}
                # UseQHPI is now determined at the OpDefCollection level
                # Set plugin collection based on package name, not individual op name
                if use_qhpi:
                    op_package_plugin_collection[resolved_op_def.package_name] = True
        return op_package_collection, op_package_plugin_collection
