...
class TensorFlowFramework_2_3_0(BaseFramework):
    ...
    def _create_connected_graph(self):
        ...
        for idx, node in enumerate(self._graph.get_operations()):
            outputs = [output.name for output in node.outputs]
            if node.type == "Placeholder":
                # If the node is a placeholder, treat it as an input and set op_type as input
                name = f"input_{idx}"
                op = FrameworkOp(name)
                op.set_inputs([])
                ...
                # For other types of nodes, use the node's name or a default name
                name = node.name or f"op_{idx}"
                op = FrameworkOp(name)
                op.set_inputs([inp.name for inp in node.inputs])
                op.set_outputs(outputs)
                op.set_op_type(node.type)
