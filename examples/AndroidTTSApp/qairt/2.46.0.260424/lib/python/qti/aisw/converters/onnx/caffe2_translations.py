...
class OnnxBboxTransformTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        log_warning("BboxTransform op {} only supports scale 1 in im_info.".format(src_op.name))
        ...
        # only batch == 1 is supported, so add zero tensor as third input
            ...
            log_assert(batch == 1,
                       "Input batch to BboxTransform op {} is expected to be 1, but got {}", src_op.name, batch)
        ...
        log_assert(params.legacy_plus_one == 0,
                   "BboxTransform op {} does not support legacy_plus_one.", src_op.name)

        # TODO: add support for rotated box in the future
        log_assert(params.rotated == 0,
                   "BboxTransform op {} does not support rotated box.", src_op.name)
...
class OnnxBoxWithNmsLimitTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        # add batch_indices
        log_assert(batch == 1,
                   "Input batch to BoxWithNmsLimit op {} is expected to be 1, but got {}", src_op.name, batch)
        ...
        log_assert(params.output_classes_include_bg_cls == 0,
                   "BoxWithNmsLimit op {} does not support output_classes_include_bg_cls.", src_op.name)

        log_assert(params.legacy_plus_one == 0,
                   "BoxWithNmsLimit op {} does not support legacy_plus_one.", src_op.name)

        # rotated BoxWithNmsLimit is not supported
        log_assert(params.rotated == 0,
                   "BoxWithNmsLimit op {} does not support rotated box.", src_op.name)

        if params.soft_nms_method not in self.SUPPORTED_NMS_KERNEL_METHODS:
            raise ValueError(
               "nms kernel method {} was not supported for BoxWithNmsLimit Op {}. Please choose from methods: {}".format(params.soft_nms_method,
                                                                                                                         src_op.name,
                                                                                                                         self.SUPPORTED_NMS_KERNEL_METHODS))
...
class OnnxDistributeFpnProposalsTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if params.legacy_plus_one != 0:
            raise ValueError(
                "The parameter 'legacy_plus_one' in DistributeFpnProposals op {} do not support non-zero values".format(src_op.name))

        if params.roi_min_level < 2 or params.roi_min_level > 5:
            raise ValueError(
                "The parameter 'roi_min_level' in DistributeFpnProposals op {} must be in range [2,5]".format(src_op.name))

        if params.roi_max_level < 2 or params.roi_max_level > 5:
            raise ValueError(
                "The parameter 'roi_max_level' in DistributeFpnProposals op {} must be in range [2,5]".format(src_op.name))

        if params.roi_max_level < params.roi_min_level:
            raise ValueError("The parameter 'roi_max_level' must be >= 'roi_min_level' in DistributeFpnProposals op {}".format(src_op.name))
...
class OnnxGenerateProposalsTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        log_assert(box_dim == 4,
                   "Rotated GenerateProposals op {} is not supported.", src_op.name)

        # raise warning that QNN only support scale 1 in im_info
        log_warning("GenerateProposals op {} only supports scale 1 in im_info.".format(src_op.name))
        ...
        log_assert(params.legacy_plus_one == 0,
                   "GenerateProposals op {} does not support legacy_plus_one.", src_op.name)
...
class OnnxIdentityTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if len(input_names) != len(output_names):
            raise RuntimeError("Identity op {} only supports same number of inputs and outputs".format(src_op.name))
...
class OnnxRoIAlignTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        # only batch == 1 is supported, so add zero tensor as third input
            ...
            log_assert(batch == 1,
                       "Input batch to RoIAlign op {} is expected to be 1, but got {}", src_op.name, batch)
            batch_idx_const_name = rois_name + '_batch_indices'
            batch_idx_const_op = op_adapter.ConstantOp(batch_idx_const_name, np.zeros(num_rois, dtype=np.int32))
            batch_idx_const_node = graph.add(batch_idx_const_op, [], [batch_idx_const_name])
            graph.add_src_op_info(batch_idx_const_node.op.name, [], [batch_idx_const_name])
        ...
        log_assert(params.spatial_scale != 0.0,
                   "RoiAlign op {} does not support zero spatial_scale.", src_op.name)
