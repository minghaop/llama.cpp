...
class ConverterBase(ABC):
    ...
    class ArgParser(ArgParserWrapper):
        def __init__(self, **kwargs):
            ConverterBase.qairt_converter = False
    ...
    class ArgParserv2(ArgParserWrapper):
        def __init__(self, **kwargs):
            ConverterBase.qairt_converter = True
    ...
    def __init__(self, args, custom_op_factory: CustomOpFactory = None):
        ...
        self.keep_int64_inputs = False
        if hasattr(args, "keep_int64_inputs"):
            self.keep_int64_inputs = args.keep_int64_inputs

        # Set keep_int64_inputs to False if preserve_io_datatype is provided since
        # preserve_io_datatype handles this independently for each IO tensor
        if hasattr(args, "preserve_io_datatype") and args.preserve_io_datatype:
            if self.keep_int64_inputs:
                log_warning("Both --keep_int64_inputs and --preserve_io_datatype options are "
                            "provided, --preserve_io_datatype will be given priority.")
            self.keep_int64_inputs = False
