...
from qti.aisw.converters.common.utils.converter_utils import log_error, log_info, log_warning, log_debug3, log_assert
...
class QnnConverterBackend(QnnConverterBackendBase):
    ...
    def add_node(self, node_name, node_type, input_names, outputs_info, tensor_params={}, scalar_params={},
        ...
        :param backend_name: the backend name to associate with this node, defaults to empty string
        """
    ...
    def save_to_dlc(self, ir_graph, source_topology):
        ...
        if source_topology:
          ...
          add_succeeded = self.dlc_serializer.add_record_from_buffer(buffer=source_topology,
    ...
    def save(self, graph):
        ...
        def get_encoding_version():
            external_quant_params = ("param_encodings" in graph.user_quantization_overrides and
                                     graph.user_quantization_overrides["param_encodings"]) or \
                                    ("activation_encodings" in graph.user_quantization_overrides and
                                     graph.user_quantization_overrides["activation_encodings"]) or \
                                    ("encodings" in graph.user_quantization_overrides and
                                     graph.user_quantization_overrides["encodings"])
            if external_quant_params:
                if "version" in graph.user_quantization_overrides and \
                        graph.user_quantization_overrides["version"] == "2.0.0":
                    return EncodingVersion.V2
                else:
                    if hasattr(self.quantizer.args, 'use_quantize_v2') and self.quantizer.args.use_quantize_v2:
                        raise Exception("Only encoding version 2.0.0 is supported with --use_quantize_v2 option")
                    return EncodingVersion.V1
            return EncodingVersion.NONE

        enc_version = get_encoding_version()

        # Direct mapping from EncodingVersion to QuantEncodingVersion
        encoding_version_map = {
            EncodingVersion.NONE: ir_graph.QuantEncodingVersion.NONE,
            EncodingVersion.V1: ir_graph.QuantEncodingVersion.V1,
            EncodingVersion.V2: ir_graph.QuantEncodingVersion.V2
        }

        # Get the quant encoding version and set it in the cpp graph
        quant_encoding_version = encoding_version_map.get(enc_version, ir_graph.QuantEncodingVersion.NONE)
        ...
        if enc_version != EncodingVersion.NONE and self.float_bitwidth == 16 and self.isBfloat16:
        ...
        enc_v2 = (enc_version == EncodingVersion.V2)
        self.quantizer.quantize(cpp_graph, self, enc_v2)
        if self.quantizer.ir_graph_reader:
            cpp_graph = self.quantizer.ir_graph_reader.get_ir_graph()
        if graph.enable_trace and self.quantizer.opts.input_list:
            cpp_graph.validation_framework_tracing_completeness("IrQuantizer")
        ...
        if self.export_format == "cpp":
            self.save_to_cpp(cpp_graph)
        elif self.export_format == "dlc":
            self.save_to_dlc(cpp_graph, graph.source_topology)
