...
static Snpe_PerformanceProfile_t perfProfile = SNPE_PERFORMANCE_PROFILE_BALANCED;
...
int main(int argc, char** argv) {
    ...
    std::string perfProfileStr = "default";
...
#else
    while ((opt = getopt(argc, argv, "hi:d:o:b:q:s:z:r:l:u:p:cx")) != -1) {
#endif
        switch (opt)
        {
            case 'h':
    ...
    perfProfile = extractPerfProfile(perfProfileStr);

    snpeHandle = setBuilderOptions(containerHandle, runtime, inputRuntimeListHandle, useUserSuppliedBuffers,
                                   platformConfigHandle, usingInitCache, cpuFixedPointMode, perfProfile);
    ...
    if (userBufferSourceType == GLBUFFER) {
      Snpe_PlatformConfig_SetIsUserGLBuffer(true);
    }
