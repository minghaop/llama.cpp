...
class QnnSampleApp {
 ...
 private:
  ...
  Qnn_TensorSet_t *createTensorSet(Qnn_Tensor_t *inputTensors,
                                   uint32_t numInputTensors,
                                   Qnn_Tensor_t *outputTensors,
                                   uint32_t numOutputTensors,
                                   Qnn_GraphHandle_t graphHandle);

  StatusCode freeTensorSet(Qnn_TensorSet_t *tensorSet, Qnn_GraphHandle_t graphHandle);

  StatusCode prepareExecutionEnvironment(Qnn_GraphHandle_t graphHandle, Qnn_TensorSet_t *tensorSet);

  StatusCode releaseExecutionEnvironment(Qnn_GraphHandle_t graphHandle, Qnn_TensorSet_t *tensorSet);
  ...
  bool m_enablePrepReleaseExecEnv{false};
