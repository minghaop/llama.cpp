...
sample_app::QnnSampleApp::~QnnSampleApp() {
  dlc_utils::freeDlcResources(m_qnnFunctionPointers.qnnSystemInterfaceHandle, m_dlcHandle, m_dlcLogHandle);
}
...
sample_app::StatusCode sample_app::QnnSampleApp::initialize() {
  ...
  if (QNN_SUCCESS == m_qnnFunctionPointers.qnnInterface.propertyHasCapability(
                         QNN_PROPERTY_GRAPH_SUPPORT_ENV_OPTION_BIND_MEM_HANDLES)) {
    QNN_VERBOSE("Backend supports the property preparing environment with mem handles");
    m_enablePrepReleaseExecEnv = true;
  }
...
sample_app::StatusCode sample_app::QnnSampleApp::composeGraphs() {
  ...
  // If DLC path is provided, use DLC-based graph composition
  if (!m_dlcPath.empty()) {
    return composeGraphsFromDlc();
  }
...
Qnn_TensorSet_t *sample_app::QnnSampleApp::createTensorSet(Qnn_Tensor_t *inputTensors,
                                                           uint32_t numInputTensors,
                                                           Qnn_Tensor_t *outputTensors,
                                                           uint32_t numOutputTensors,
                                                           Qnn_GraphHandle_t graphHandle) {
  Qnn_TensorSet_t *tensorSet = (Qnn_TensorSet_t *)malloc(sizeof(Qnn_TensorSet_t));
  if (tensorSet == nullptr) {
    QNN_ERROR("Failed to allocate memory for tensor set");
    return nullptr;
  }
  *tensorSet = QNN_TENSORSET_CREATE(QNN_TENSOR_SET_VERSION_1);
  QNN_TENSORSET_SET_INPUT_TENSORS(tensorSet, inputTensors, numInputTensors);
  QNN_TENSORSET_SET_OUTPUT_TENSORS(tensorSet, outputTensors, numOutputTensors);
  if (StatusCode::SUCCESS != prepareExecutionEnvironment(graphHandle, tensorSet)) {
    free(tensorSet);
    return nullptr;
  }
  return tensorSet;
...
sample_app::StatusCode sample_app::QnnSampleApp::freeTensorSet(Qnn_TensorSet_t *tensorSet,
                                                               Qnn_GraphHandle_t graphHandle) {
  if (tensorSet == nullptr) {
    QNN_ERROR("input tensor set for freeing is already null");
    return StatusCode::FAILURE;
  }
  // Just discard the return value of releaseExecutionEnvironment as it is of no use.
  releaseExecutionEnvironment(graphHandle, tensorSet);
  free(tensorSet);
  return StatusCode::SUCCESS;
}

sample_app::StatusCode sample_app::QnnSampleApp::prepareExecutionEnvironment(
    Qnn_GraphHandle_t graphHandle, Qnn_TensorSet_t *tensorSet) {
  if (nullptr != m_qnnFunctionPointers.qnnInterface.graphPrepareExecutionEnvironment) {
    QnnGraph_ExecuteEnvironment_t execEnvInstance;
    execEnvInstance.option    = QNN_GRAPH_EXECUTE_ENVIRONMENT_OPTION_BIND_MEM_HANDLES;
    execEnvInstance.tensorSet = *tensorSet;
    uint32_t numExecEnvs      = 1;
    QnnGraph_ExecuteEnvironment_t *execEnv[] = {&execEnvInstance};
    if (QNN_SUCCESS != m_qnnFunctionPointers.qnnInterface.graphPrepareExecutionEnvironment(
                           graphHandle, execEnv, numExecEnvs)) {
      QNN_ERROR("Unable to prepare execution environment");
      return StatusCode::FAILURE;
    }
  }
  return StatusCode::SUCCESS;
...
sample_app::StatusCode sample_app::QnnSampleApp::releaseExecutionEnvironment(
    Qnn_GraphHandle_t graphHandle, Qnn_TensorSet_t *tensorSet) {
  if (nullptr != m_qnnFunctionPointers.qnnInterface.graphReleaseExecutionEnvironment) {
    QnnGraph_ExecuteEnvironment_t execEnvInstance;
    execEnvInstance.option    = QNN_GRAPH_EXECUTE_ENVIRONMENT_OPTION_BIND_MEM_HANDLES;
    execEnvInstance.tensorSet = *tensorSet;
    uint32_t numExecEnvs      = 1;
    const QnnGraph_ExecuteEnvironment_t *execEnv[] = {&execEnvInstance};
    if (QNN_SUCCESS != m_qnnFunctionPointers.qnnInterface.graphReleaseExecutionEnvironment(
                           graphHandle, execEnv, numExecEnvs)) {
      QNN_ERROR("Unable to release execution environment");
      return StatusCode::FAILURE;
    }
  }
  return StatusCode::SUCCESS;
...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphs() {
  ...
  for (unsigned int run = 0; run < m_numInferences; run++) {
    for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
      ...
      Qnn_TensorSet_t *tensorSet = nullptr;
      ...
      if (m_enablePrepReleaseExecEnv) {
        tensorSet = createTensorSet(inputs,
                                    graphInfo.numInputTensors,
                                    outputs,
                                    graphInfo.numOutputTensors,
                                    graphInfo.graph);
        if (tensorSet == nullptr) {
          QNN_ERROR("Failed to create tensor set for graphIdx: %d", graphIdx);
      ...
      if (m_enablePrepReleaseExecEnv) {
        freeTensorSet(tensorSet, graphInfo.graph);
      }
      m_ioTensor.tearDownInputAndOutputTensors(
          inputs, outputs, graphInfo.numInputTensors, graphInfo.numOutputTensors);
      inputs  = nullptr;
