...
int32_t pal::FileOp::open(const std::string &path,
  ...
  if (!(static_cast<int32_t>(pmode) & ~static_cast<int32_t>(pal::FileOp::FileMode::S_IRUSR_ |
                                                            pal::FileOp::FileMode::S_IRGRP_ |
  ...
  if (static_cast<int32_t>(flags & pal::FileOp::AccessMode::O_APPEND_)) {
    ...
    if(_lseek(fd, 0, SEEK_END) == -1L) {
      DEBUG_MSG("Failed to mode file descriptor pointer to the end for append mode! Error code: %d", errno);
      CloseHandle(fileHandle);
      pal::FileOp::close(fd);
      return -1;
    }
