...
#include "QnnContextConfig.hpp"

void QnnContextConfig::serialize(nlohmann::ordered_json& json) const {
  ...
  switch (m_config.option) {
    case QNN_CONTEXT_CONFIG_OPTION_CUSTOM:
      ...
      json["customConfig"] = nlohmann::ordered_json();  // Populated by subclasses
      break;
...
void ContextCustomHtpConfig::serialize(nlohmann::ordered_json& json) const {
  QnnContextConfig::serialize(json);
  nlohmann::ordered_json& customConfigJson = json["customConfig"];
