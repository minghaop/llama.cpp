...
class MmappedReader {
 public:
  explicit MmappedReader(std::shared_ptr<BufferLike> mmappedFile, bool littleEndian = true)
  ...
  bool isOpen() const noexcept { return m_mmappedFile && *m_mmappedFile; }
  ...
  explicit operator bool() const noexcept { return isOpen() && !fail(); }
 ...
 public:
  ...
  bool get(T& dest) {
    ...
    auto data = current_const<uint8_t>();
  ...
  bool read(uint8_t* dst, uint64_t bytes) {
    ...
    step(static_cast<int64_t>(bytes));  // This will succeed if get succeeds
    return true;
  }
 ...
 private:
  ...
  bool read(std::string& str) {
    return read(reinterpret_cast<uint8_t*>(&str[0]), static_cast<uint64_t>(str.size()));
  }

  // Read into a vector and advance the stream
  template <typename T>
  bool read(std::vector<T>& vec) {
    return read(reinterpret_cast<uint8_t*>(vec.data()),
                static_cast<uint64_t>(vec.size()) * sizeof(T));
  }

  // Get the current data, reinterpreted as a specified type
  template <typename T>
  const T* current_const() const noexcept {
    return reinterpret_cast<const T*>(m_mmappedFile->data() + m_offset);
  }
