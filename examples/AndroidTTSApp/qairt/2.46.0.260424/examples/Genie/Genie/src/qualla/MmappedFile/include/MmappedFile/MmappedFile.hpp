...
inline bool isLittle() noexcept {
  ...
  return (*reinterpret_cast<const uint8_t*>(&n)) == expectedValue;
}
...
// A plain data buffer
class DataBuffer final : public BufferLike {
 public:
  ...
  explicit operator bool() const noexcept override { return (m_data != nullptr) && (m_size != 0); }
...
class File final : public BufferLike {
 public:
