...
bool File::resize(uint64_t newSize) {
  if (!m_readWrite) {
    ...
    if (m_size != newSize && newSize != 0) {
      if (ftruncate(m_fileDescriptor, static_cast<off_t>(newSize)) != 0) {
...
std::pair<void*, uint64_t> File::getRange(uint64_t offset, uint64_t length) {
  ...
  return {static_cast<void*>((static_cast<uint8_t*>(m_address)) + start), len};
}
...
std::pair<int, uint64_t> File::openFileInternal(const char* filename,
                                                int openflags,
                                                int modeFlags) {
...
#ifdef __hexagon__
  std::ignore = size;
  std::ignore = readwrite;
  return {nullptr, 0};
#else
  void* addr = nullptr;
  if (size > 0) {
    const int prot  = readwrite ? static_cast<int>(static_cast<unsigned int>(PROT_READ) |
                                                  static_cast<unsigned int>(PROT_WRITE))
                                : PROT_READ;
    const int flags = readwrite ? static_cast<int>(static_cast<unsigned int>(MAP_SHARED))
                                : static_cast<int>(static_cast<unsigned int>(MAP_PRIVATE));
    //    flags |= MAP_HUGE_2MB; // Does not significantly improve performance
    addr = mmap(nullptr, static_cast<size_t>(size), prot, flags, m_fileDescriptor, {});
  }
...
#if defined(USE_POSIX_MADVISE) || \
    defined(NO_MADVISE_SUPPORT)  // TODO: Should have more specific macro for remap
...
bool File::open(bool readwrite, uint64_t overrideSize) {
  ...
  if (!readwrite && (overrideSize != 0)) {
    return false;
  }

  const int openflags =
      readwrite
          ? static_cast<int>(static_cast<unsigned int>(O_RDWR) | static_cast<unsigned int>(O_CREAT))
          : O_RDONLY;
  const int modeFlags = readwrite ? static_cast<int>(static_cast<unsigned int>(S_IRUSR) |
                                                     static_cast<unsigned int>(S_IWUSR))
                                  : 0;
  uint64_t openedSize{};
  std::tie(m_fileDescriptor, openedSize) =
      openFileInternal(m_filename.c_str(), openflags, modeFlags);
  if (m_fileDescriptor == -1) {
    return false;
  }

  if (overrideSize != 0 && openedSize != overrideSize) {
    if (ftruncate(m_fileDescriptor, static_cast<off_t>(overrideSize)) != 0) {
      return false;
    }
  }

  std::tie(m_address, m_size) =
      mapFileInternal((overrideSize != 0) ? overrideSize : openedSize, readwrite);
...
bool File::unmapInternal() {
  ...
  if (m_address != nullptr) {
    if (munmap(m_address, static_cast<size_t>(m_size)) != 0) {
      toret = false;
    }
    m_address = nullptr;
    m_size    = 0;
...
bool File::open() {
  ...
  m_fileHandle = CreateFileA(
      ...
      FILE_SHARE_READ | FILE_SHARE_WRITE |
          FILE_SHARE_DELETE,    // dwSharedMode with shared delete to mimic Linux behavior
