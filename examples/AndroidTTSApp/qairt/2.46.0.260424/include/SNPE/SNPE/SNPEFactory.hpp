...
/**
 * @brief Cleans up runtime/backend resources. It is recommended to call this API when all
 * SNPE instaces have de-initialized.
 *
 * @warning cleanupRuntime() must not be called while another SNPE instance is still active
 * or executing in the same process. In a multi-threaded use case, the individual threads
 * must have a cooperative life cycle management strategy for managing backend resources.
 * Once this API is called subsequent initialization of SNPE instance using this runtime will
 * be slower (similar to first time initialization)
 *
 * @return True if successful, False otherwise.
 */
  static bool cleanupRuntime(DlSystem::Runtime_t runtime, uint32_t deviceId = 0){
    return Snpe_Util_CleanupRuntime(static_cast<Snpe_Runtime_t>(runtime), deviceId);
  }
