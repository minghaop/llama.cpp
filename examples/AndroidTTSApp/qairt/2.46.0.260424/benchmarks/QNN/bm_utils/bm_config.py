...
class Config(object):
    def __init__(self, cfg_file, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype,
        ...
        if deviceostype and deviceostype not in CONFIG_VALID_DEVICEOSTYPES:
        ...
        elif deviceostype == self._product.CONFIG_DEVICEOSTYPES_AARCH64_LINUX_OPENWRT_GCC133:
            self._architectures = [self._product.ARCH_AARCH64]
            self._compiler = self._product.COMPILER_CLANG90
            self._platform_os = self._product. PLATFORM_OS_ANDROID
            self._stl_library = self._product.STL_LIBCXX_SHARED
            self._artifacts_config = load_json(
                os.path.join(
                    config_prefix,
                    product.PRODUCT_BM,
                    self._product.BENCH_AARCH64_LINUX_OPENWRT_GCC133_ARTIFACTS_JSON))
        elif deviceostype == self._product.CONFIG_DEVICEOSTYPES_ANDROID_ARM32:
            ...
            self._artifacts_config = load_json(
                os.path.join(
                    config_prefix,
                    product.PRODUCT_BM,
                    self._product.BENCH_ANDROID_ARM_ARTIFACTS_JSON))
    ...
    def artifacts(self):
        arch_map = {
            ...
            'arm-android': 'arm32-v7a',
            'aarch64-openwrt-linux-gcc13.3': 'dummy',
