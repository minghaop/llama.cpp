...
class Config(object):
    def __init__(self, cfg_file, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype, userbuffer_mode, perfprofile, profilinglevel, cdsp, enable_init_caching):
        ...
        elif deviceostype == CONFIG_DEVICEOSTYPES_ANDROID_ARM32:
            self._architectures = [ARCH_ARM]
            self._platform_os = PLATFORM_OS_ANDROID
            self._stl_library = STL_LIBCXX_SHARED
            if (len(self.pt)>0):
                self._artifacts_config = SNPE_BENCH_ANDROID_ARM32_ARTIFACTS.replace(SNPE_BENCH_ANDROID_ARM32_ARTIFACTS[SNPE_BENCH_ANDROID_ARM32_ARTIFACTS.rfind("-"):],"")
                self._compiler = ""
            else:
        ...
        elif deviceostype == CONFIG_DEVICEOSTYPES_AARCH64_OPENWRT_LINUX_GCC133:
            self._architectures = [ARCH_AARCH64]
            self._platform_os = PLATFORM_OS_LINUX
            self._compiler = COMPILER_GCC133
            self._stl_library = None
            self._artifacts_config = SNPE_BENCH_AARCH64_OPENWRT_LINUX_GCC133_ARTIFACTS
