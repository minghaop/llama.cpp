...
// flow encoder layer weight
struct enc_layer_weights {
    ggml_tensor *norm_mha_w, *norm_mha_b;
    ggml_tensor *wq, *bq, *wk, *bk, *wv, *bv, *wpos;
    ggml_tensor *pos_bias_u, *pos_bias_v;
    ggml_tensor *wo, *bo;
    ggml_tensor *norm_ffn_w, *norm_ffn_b;
    ggml_tensor *ffn_w1, *ffn_b1, *ffn_w2, *ffn_b2;
};

// mask_to_bias 自定义算子
struct mask_to_bias_params {
    float neg_val = -1.0e10f;
};

// sinusoidal_pos_emb 自定义算子
struct sinusoidal_params {
    int dim = 320;
    int scale = 1000;
};
...
// 统一的基础注意力块
struct AttnWeights {
    ggml_tensor *wq, *wk, *wv, *wo, *bo;
};

struct BlockWeights {
    ggml_tensor *conv_w, *conv_b, *norm_w, *norm_b;
    ggml_tensor *mlp_w, *mlp_b, *res_w, *res_b;
    ggml_tensor *conv2_w, *conv2_b, *norm2_w, *norm2_b;
};
...
struct TransformerBlockWeights {
    ...
    ggml_tensor *ffn_w0, *ffn_b0, *ffn_w2, *ffn_b2;
};
