...
llama_ubatch llama_batch_allocr::ubatch_reserve(uint32_t n_seq_tokens, uint32_t n_seqs) {
    ...
    udata->flow_token.resize(udata->token_len + udata->prompt_token_len);
    udata->flow_feat .resize(udata->prompt_feat_len);
    ...
    llama_ubatch res {
        ...
        //CosyVoiceFlows
        /*flow_token    =*/ udata->flow_token.data(),
        /*flow_feat     =*/ udata->flow_feat.data(),
        /*token_len     =*/ udata->token_len,
        /*prompt_token_len =*/ udata->prompt_token_len,
...
llama_ubatch llama_batch_allocr::ubatch_add(const std::vector<int32_t> & idxs, uint32_t n_seqs, bool equal_seqs) {
    ...
    if (batch.flow_token  == nullptr && batch.embd) {
        ...
        udata->embd      .resize(n_embd_all);
        udata->pos       .resize(n_pos_all);
        udata->n_seq_id  .resize(flow_total_len);
        udata->seq_id    .resize(flow_total_len);
        udata->seq_id_unq.resize(0);
        udata->seq_idx   .resize(LLAMA_MAX_SEQ, -1);
        udata->output    .resize(flow_total_len);
    ...
    for (size_t i = 0; i < idxs.size(); ++i) {
        ...
        if (batch.flow_token) {
            if (batch.embd) {
                memcpy(udata->embd.data() + i, batch.embd + (int64_t) idxs[i], sizeof(float));
            }
        }else {
            if (batch.embd) {
    ...
    if (batch.flow_token) {
        memcpy(udata->flow_token.data(), batch.flow_token, flow_token_data_len * sizeof(float));
    }
    
    if (batch.flow_feat) {
        memcpy(udata->flow_feat.data(), batch.flow_feat, batch.prompt_feat_len * sizeof(float));
    }
    
    if (batch.rand_noise) {
        memcpy(udata->rand_noise.data(), batch.rand_noise, 80 * 50 * 300 * sizeof(float));
    }
    if (batch.extend_pe) {
        memcpy(udata->extend_pe.data(), batch.extend_pe, 9999 * 512 * sizeof(float));
    }
    ...
    llama_ubatch res {
        ...
        /*.seq_idx      =*/ udata->seq_idx.data(),
        /*.output       =*/ udata->output.data(),
        /*flow_token    =*/ batch.flow_token ? udata->flow_token.data() : nullptr,
        /*flow_feat     =*/ batch.flow_feat ? udata->flow_feat.data() : nullptr,
