...
static int llama_model_load(const std::string & fname, std::vector<std::string> & splits, llama_model & model, llama_model_params & params) {
    ...
    try {
        ...
        if(ml.get_arch() == LLM_ARCH_COSYVOICEFLOW) {
            model.hparams.use_flow = true;
            model.hparams.use_hift = false;
        } else if (ml.get_arch() == LLM_ARCH_COSYVOICEHIFT) {
            model.hparams.use_flow = false;
            model.hparams.use_hift = true;
        }

        LLAMA_LOG_INFO("&&&&&&&&&&&&&&&&&&&&&&&&&& flow status is: %d, hift status is: %d\n", params.is_flow ? 1: 0, params.is_hift ? 1: 0);
        LLAMA_LOG_INFO("&&&&&&&&&&&&&&&&&&&&&&&&&& flow status is: %d, hift status is: %d\n", model.hparams.use_flow ? 1: 0, model.hparams.use_hift ? 1: 0);
        try {
            model.load_arch(ml);
            ...
            switch (ggml_backend_dev_type(dev)) {
                ...
                case GGML_BACKEND_DEVICE_TYPE_GPU: {
                    ...
                    if (ggml_backend_reg_name(reg) == std::string("RPC")) {
                        ...
                        LLAMA_LOG_INFO("GPU设备: %s, 最大命令缓冲区: %d", props.name, props.memory_total);
