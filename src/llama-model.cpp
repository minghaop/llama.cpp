            ...
            case LLM_ARCH_COSYVOICEFLOW:
                {
                    ...
                    for(int i = 280; i < 292; ++i) {
                        ...
                        for(int j = 0; j < 4; ++j) {
                            ...
                            sub_layer.mid_block1_norm3_b = create_tensor(tn(LLM_TENSOR_MID_BLOCKS1_NORM3_BIAS, "bias", i - 280, j), {256}, 0);
...
struct llm_build_flow : public llm_graph_context {
    ...
    llm_build_flow(const llama_model & model, const llm_graph_params & params, ggml_cgraph * gf) : llm_graph_context(params), model(model) {
        ...
        ggml_tensor * token = expand(build_inp_token());
        ggml_tensor * prompt_feat = expand(build_inp_prompt_feat());
        ggml_tensor * extend_pe = expand(build_inp_extend_pe());
        
        ggml_tensor * spk_add = expand(ggml_add(ctx0, ggml_mul_mat(ctx0, model.spk_embed_w, embedding), model.spk_embed_b));

        int len = params.ubatch.prompt_token_len + params.ubatch.token_len;
        ggml_tensor * mask = ggml_cont(ctx0, ggml_permute(ctx0, build_pad_mask(len, 0, 0), 1, 0, 2, 3));
        ggml_tensor * token_mask = ggml_mul(ctx0, build_flow_embedding(token, model.inp_embed_w, -1), mask);

        ggml_tensor * x = build_espnet_pos_encode(
                            build_linear_no_subsampling(token_mask, model.embed_out_0_w, model.embed_out_0_b,
                                                        model.embed_out_1_w, model.embed_out_1_b, 1), 1);
        ggml_tensor * pos_emb = build_pos_encoding(ggml_dup(ctx0, extend_pe), x->ne[1], 0, 1);
        x = expand(build_pre_lookahead_layer(x, model.pre_look_conv1_w, model.pre_look_conv1_b,
                                         model.pre_look_conv2_w, model.pre_look_conv2_b));
        
        // 6 encoder layers
        for (int i = 0; i < 6; i++) {
            auto & L = model.layers[i + 13];
            enc_layer_weights W = {
                L.encoders_normmha_w, L.encoders_normmha_b,
                L.encoders_wq, L.encoders_bq, L.encoders_wk, L.encoders_bk, L.encoders_wv, L.encoders_bv,
                L.encoders_wpos, L.encoders_pos_bias_u, L.encoders_pos_bias_v,
                L.encoders_wo, L.encoders_bo,
                L.encoders_normffn_w, L.encoders_normffn_b,
                L.encoders_ffn_w1, L.encoders_ffn_b1, L.encoders_ffn_w2, L.encoders_ffn_b2
            };
            x = build_encoder_layer(gf, x, pos_emb, mask, W, i);
        }
        ggml_set_name(x, "after_encoders");
        x = build_upsample_1d(gf, x, model.up_layer_conv_w, model.up_layer_conv_b);
        x = ggml_cont(ctx0, ggml_permute(ctx0, x, 1, 0, 2, 3));
        
        x = build_espnet_pos_encode(
            build_linear_no_subsampling(x, model.up_embed_out_0_w, model.up_embed_out_0_b,
                                        model.up_embed_out_1_w, model.up_embed_out_1_b, 2), 2);
        pos_emb = build_pos_encoding(ggml_dup(ctx0, extend_pe), x->ne[1], 0, 2);
        
        // 4 up-encoder layers
        for (int i = 0; i < 4; i++) {
            auto & L = model.layers[i + 132];
            enc_layer_weights W = {
                L.up_encoders_normmha_w, L.up_encoders_normmha_b,
                L.up_encoders_wq, L.up_encoders_bq, L.up_encoders_wk, L.up_encoders_bk, 
                L.up_encoders_wv, L.up_encoders_bv, L.up_encoders_wpos,
                L.up_encoders_pos_bias_u, L.up_encoders_pos_bias_v,
                L.up_encoders_wo, L.up_encoders_bo,
                L.up_encoders_normffn_w, L.up_encoders_normffn_b,
                L.up_encoders_ffn_w1, L.up_encoders_ffn_b1, L.up_encoders_ffn_w2, L.up_encoders_ffn_b2
            };
            x = build_encoder_layer(gf, x, pos_emb, mask, W, i + 6);
        }

        // Final norm
        x = build_layer_norm(x, model.after_norm_w, model.after_norm_b, 1e-5f, "final", 0);
        ggml_set_name(x, "after_up_encoders");
        // encoder projection
        ...
        //     cb(scores, "scores", i);
        //     ggml_tensor * x_att = build_attn_scores(value, scores, mask, model.layers[i + 13].encoders_wo, model.layers[i + 13].encoders_bo, "encoders", i);
        //     cb(x_att, "x_att", i);
        ...
        //     ggml_tensor * x_att = build_attn_scores(value, scores, mask, model.layers[i + 132].up_encoders_wo, model.layers[i + 132].up_encoders_bo, "up_encoders", i);
        //     cb(x_att, "up_x_att", i);
        //     x = ggml_add(ctx0, attn_residual, x_att);
        //     ggml_tensor * ffn_residual = ggml_dup(ctx0, x);
        //     ggml_build_forward_expand(gf, ffn_residual);
