...
private:
    ...
    bool load_model(const common_params & params) {
        ...
        for (int i = 0; i < params_base.n_parallel; i++) {
            ...
            if (model_dft) {
                slot.batch_spec = llama_batch_init(params_base.speculative.n_max + 1, 0, 1, 0);
        ...
        // note that n_batch can be > n_ctx (e.g. for non-causal attention models such as BERT where the KV cache is not used)
        {
            ...
            batch = llama_batch_init(std::max(n_batch, params_base.n_parallel), 0, 1, 0);
        }
    ...
    bool launch_slot_with_task(server_slot & slot, server_task && task) {
        ...
        if (slot.ctx_dft) {
            ...
            slot.batch_spec = llama_batch_init(task.params.speculative.n_max + 1, 0, 1, 0);
        }
